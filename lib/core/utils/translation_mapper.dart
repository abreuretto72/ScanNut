...
class TranslationMapper {
  ...
  static String localizeFoodName(String rawName, AppLocalizations l10n) {
    ...
    if (l10n.localeName.startsWith('en')) {
      ...
      if (key.contains('rap10') || key.contains('tortilha')) {
      ...
      if (key.contains('vitamina de abacate')) {
        return 'Avocado Smoothie with Oats';
      }
    ...
    if (key.contains('feijão') ||
        key.contains('feijao') ||
        key.contains('beans')) {
      return l10n.foodBeans;
    }
    if (key.contains('frango') ||
        key.contains('chicken') ||
        key.contains('pollo')) {
      return l10n.foodChicken;
    }
    if (key.contains('carne') ||
        key.contains('meat') ||
        key.contains('carne de res')) {
      return l10n.foodMeat;
    }
    if (key.contains('peixe') ||
        key.contains('fish') ||
        key.contains('pescado')) {
      return l10n.foodFish;
    }
    if (key.contains('ovo') || key.contains('egg') || key.contains('huevo')) {
      return l10n.foodEgg;
    }
    if (key.contains('pão') ||
        key.contains('pao') ||
    ...
    if (key.contains('leite') || key.contains('milk') || key.contains('leche')) {
      return l10n.foodMilk;
    }
    if (key.contains('café') || key.contains('cafe') || key.contains('coffee')) {
      return l10n.foodCoffee;
    }
    if (key.contains('iogurte') ||
        key.contains('yogurt') ||
        key.contains('yogur')) {
      return l10n.foodYogurt;
    }
    if (key.contains('fruta') || key.contains('fruit')) return l10n.foodFruit;
    if (key.contains('salada') ||
        key.contains('salad') ||
        key.contains('ensalada')) {
      return l10n.foodSalad;
    }
    if (key.contains('legume') ||
        key.contains('vegetable') ||
        key.contains('verdura')) {
      return l10n.foodVegetables;
    }
    if (key.contains('macarrão') ||
        key.contains('macarrao') ||
        key.contains('pasta')) {
      return l10n.foodPasta;
    }
    if (key.contains('batata') ||
        key.contains('potato') ||
        key.contains('papa')) {
      return l10n.foodPotato;
    }
    if (key.contains('mandioca') ||
        key.contains('cassava') ||
        key.contains('yuca')) {
      return l10n.foodCassava;
    }
    if (key.contains('queijo') ||
        key.contains('cheese') ||
        key.contains('queso')) {
      return l10n.foodCheese;
    }
    if (key.contains('suco') || key.contains('juice') || key.contains('zumo')) {
      return l10n.foodJuice;
    }
    if (key.contains('água') || key.contains('agua') || key.contains('water')) {
      return l10n.foodWater;
    }

    // New mappings for common ingredients
    if (key.contains('maçã') || key.contains('maca') || key.contains('apple')) {
    ...
    if (key.contains('cenoura') || key.contains('carrot')) {
    ...
    if (key.contains('laranja') || key.contains('orange')) {
      return l10n.foodOrange;
    }
    if (key.contains('abacaxi') || key.contains('pineapple')) {
      return l10n.foodPineapple;
    }
