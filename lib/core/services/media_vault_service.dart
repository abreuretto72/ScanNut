...
class MediaVaultService {
      ...
      if (await medicalDocs.exists()) {
        ...
        for (var entity in contents) {
          if (entity is Directory) {
            ...
            final destPetDir =
                Directory('${vaultRoot.path}/$PETS_DIR/$petName');
            if (!await destPetDir.exists()) {
              await destPetDir.create(recursive: true);
            }
            ...
            for (var f in files) {
              if (f is File) {
                try {
                  final destPath =
                      '${destPetDir.path}/${path.basename(f.path)}';
                  if (!File(destPath).existsSync()) {
                    await f.copy(destPath);
                    // Optional: f.delete(); // Keep legacy for safety for now
                    debugPrint(
                        '📦 Migrated pet file: ${path.basename(f.path)}');
                  }
                } catch (e) {
  ...
  Future<String> secureClone(File sourceFile, String category,
      [String? subFolder, bool skipIndexing = false]) async {
    final appDir = await getApplicationSupportDirectory();
    ...
    if (isImage &&
        !safeFilename.toLowerCase().endsWith('.jpg') &&
    ...
    if (isImage) {
      ...
      debugPrint(
          '📄 Copying ${isPdf ? "PDF" : "document"} to Vault: $safeFilename');
      await sourceFile.copy(destFile.path);
    }
    ...
    if (!await destFile.exists()) {
      throw Exception(
          "Vault Clone Failed: File not found after copy at ${destFile.path}");
    }
    ...
    debugPrint(
        '✅ Vault Secure Clone Success: ${destFile.path} ($length bytes)');
  ...
  Future<String> attemptRecovery(String filePath, {String? category}) async {
    ...
    if (await file.exists()) {
      debugPrint(
          '   [VAULT_TRACE] ✅ File actually exists at original path. No recovery needed.');
      return filePath;
    } else {
    ...
    try {
      ...
      debugPrint(
          '   [VAULT_TRACE] Checking Internal Vault at: ${vaultRoot.path}');

      if (await vaultRoot.exists()) {
        final matches = vaultRoot
            .listSync(recursive: true)
            .where((fs) => path.basename(fs.path) == filename);
        debugPrint('   [VAULT_TRACE] Vault matches found: ${matches.length}');

        if (matches.isNotEmpty) {
          final recoveredPath = matches.first.path;
          debugPrint(
              '✅ [VAULT_TRACE] FOUND in Vault! Recovered path: $recoveredPath');
    ...
    try {
      ...
      debugPrint(
          '   [VAULT_TRACE] Checking Backup Mirror at: ${backupRoot.path}');

      // Deep search in backup root
      final matches = backupRoot
          .listSync(recursive: true)
          .where((fs) => path.basename(fs.path) == filename);
      ...
      if (matches.isNotEmpty) {
        ...
        final vaultDir = Directory(
            '${(await getApplicationSupportDirectory()).path}/$VAULT_ROOT/$targetCat');
        ...
        debugPrint(
            '✨ [VAULT_TRACE] SELF-HEALING SUCCESS: Restored to $destPath');
        return destPath;
      } else {
  ...
  Future<Directory> _getPublicBackupRoot() async {
    ...
    final backup = Directory(
        '${root.path}/ScanNut_Safe_Backup'); // Updated as per "Intervention" prompt
    if (!await backup.exists()) await backup.create();
    return backup;
  }

  Future<Directory> _getPublicBackupDir(String category,
