...
class ExportService {
  ...
  // --- DOMAIN COLORS ---
  static final PdfColor colorPet =
      PdfColor.fromHex('#FFD1DC'); // Rosa Pastel (Protocol V62)
  static final PdfColor colorPetLight =
      PdfColor.fromHex('#FFE4E9'); // Lighter pink
  static final PdfColor colorPetUltraLight =
      PdfColor.fromHex('#FFF5F7'); // Very light pink
  ...
  Future<pw.ImageProvider?> safeLoadImage(String? path) async {
    ...
    try {
      ...
      debugPrint(
          '🔄 [V70.1-PDF] Loading optimized image: ${path.split('/').last}');

      final optimizedBytes =
          await ImageOptimizationService().loadOptimizedBytes(
        originalPath: path,
      ...
      if (optimizedBytes != null) {
        debugPrint(
            '✅ [V70.1-PDF] Image optimized: ${(optimizedBytes.length / 1024).toStringAsFixed(2)} KB');
        ...
        child: pw.Text(
          ...
          style: pw.TextStyle(
              ...
              color: textColor ??
                  ((color == colorPet)
                      ? PdfColors.black
                      : (color != null ? PdfColors.white : PdfColors.black))),
        ),
      ),
    );
  }
  ...
  pw.Widget buildHeader(String title, String timestamp,
      {String dateLabel = 'Data', PdfColor? color}) {
    return pw.Container(
              ...
              if (cleaned.isEstimated) ...[
                ...
                pw.Text('*',
                    style: pw.TextStyle(
                        fontSize: 10,
                        fontWeight: pw.FontWeight.bold,
                        color: PdfColors.black)), // Black asterisk
              ]
            ],
          ),
          pw.SizedBox(height: 2),
          pw.Text(cleaned.value,
              style: pw.TextStyle(
                  fontSize: 14,
                  fontWeight: pw.FontWeight.bold,
  ...
  Iterable<pw.Widget> _buildObservationsBlock(
      String observations, AppLocalizations strings) sync* {
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        header: (context) => buildHeader(strings.pdfAgendaReport, timestampStr,
            color: colorPet, dateLabel: strings.pdfDateLabel),
        ...
        build: (context) => [
          pw.Row(
            ...
            children: [
              ...
              buildIndicator('${strings.pdfTotalFound}:',
                  partners.length.toString(), PdfColors.blue700),
            ],
          ),
  ...
  Future<pw.Document> generateRadarReport({
    ...
    pdf.addPage(
      pw.MultiPage(
        pageFormat: PdfPageFormat.a4,
        margin: const pw.EdgeInsets.all(35),
        header: (context) => buildHeader(strings.pdfRadarTitle, timestampStr,
            color: colorPet, dateLabel: strings.pdfDateLabel),
        ...
        build: (context) => [
          pw.Row(
            ...
            children: [
              buildIndicator(
                  '${strings.pdfStatus}:', strings.pdfRadarResults, colorPet),
              buildIndicator('${strings.pdfTotalFound}:',
                  partners.length.toString(), colorPet),
            ],
          ),
          pw.SizedBox(height: 25),
          pw.TableHelper.fromTextArray(
            ...
            headers: [
              ...
              strings.ratingLabel.toUpperCase() ??
                  (strings.localeName == 'pt' ? 'AVALIAÇÃO' : 'RATING'),
              strings.pdfAddressLabel.toUpperCase(),
              strings.pdfPhone.toUpperCase()
            ],
            data: partners.map((p) {
              final dist = PartnerService()
  ...
  Future<pw.Document> generateWeeklyMenuReport({
    ...
    List<dynamic>?
        recommendedBrands, // NEW: Brand suggestions (can be String or BrandSuggestion)
  }) async {
    debugPrint('🚀 [V62-TRACE] Iniciando build do PDF: Nome do Pet: $petName');
    debugPrint(
        '🎨 [V62-TRACE] Carregando cores do tema de Domínio Pet: Rosa Pastel (#FFD1DC)');
    debugPrint(
        '📊 [V62-TRACE] Verificando dados do cardápio: ${plan.length} itens encontrados');
    ...
    pdf.addPage(
      pw.MultiPage(
          ...
          header: (context) => buildHeader(
              strings.pdfNutritionSection, timestampStr,
              color: colorPet, dateLabel: strings.pdfDateLabel),
          ...
          build: (context) {
              ...
              child: pw.Column(
                children: [
                  pw.Row(
                    mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
                    children: [
                      pw.Text('${strings.pdfFieldPet.toUpperCase()}: $petName',
                          style: pw.TextStyle(
                              fontWeight: pw.FontWeight.bold,
                              fontSize: 13,
                              color: PdfColors.black)),
                      pw.Text(
                          '${strings.pdfFieldPeriod.toUpperCase()}: ${period ?? strings.pdfPeriodWeekly}',
                          style: pw.TextStyle(
                              fontWeight: pw.FontWeight.bold,
                              fontSize: 10,
                              color: PdfColors.black)),
                  ...
                  pw.Row(
                    ...
                    children: [
                      pw.Text(
                          '${strings.pdfFieldBreed.toUpperCase()}: $raceName',
                          style: const pw.TextStyle(
                              fontSize: 10, color: PdfColors.black)),
                      pw.Text(
                          '${strings.pdfFieldRegime.toUpperCase()}: $dietType',
                          style: pw.TextStyle(
                              color: PdfColors.black,
                              fontWeight: pw.FontWeight.bold,
                              fontSize: 10)),
                  ...
                  if (dailyKcal != null) ...[
                    ...
                    pw.Align(
                      ...
                      child: pw.Text(
                          '${strings.pdfFieldDailyKcalMeta.toUpperCase()}: $dailyKcal',
                          style: pw.TextStyle(
                              color: PdfColors.black,
                              fontWeight: pw.FontWeight.bold,
                              fontSize: 10)),
                  ...
                  child: pw.Column(
                    ...
                    children: [
                        ...
                        return pw.Container(
                          ...
                          child: pw.Column(
                            ...
                            children: [
                              ...
                              pw.SizedBox(height: 10),
                              pw.Text(strings.pdfFieldDetailsComposition,
                                  style: pw.TextStyle(
                                      fontSize: 7,
                                      fontWeight: pw.FontWeight.bold,
                                      color: PdfColors.black)),
                              pw.SizedBox(height: 4),
                              pw.Text(meal['descricao']?.toString() ?? '',
                                  style: const pw.TextStyle(
                      ...
                      if (meals.isEmpty)
                        pw.Padding(
                            padding: const pw.EdgeInsets.all(10),
                            child: pw.Text(strings.pdfNoMealsPlanned,
                                style: pw.TextStyle(
                                    fontStyle: pw.FontStyle.italic,
                                    fontSize: 9,
                                    color: PdfColors.black))),
            ...
            if (plan.isNotEmpty) {
              currentId = plan.first['planId'] as String? ??
                  plan.first['plan_id'] as String?;
            }

            for (var item in plan) {
              final id =
                  item['planId'] as String? ?? item['plan_id'] as String?;
              if (id != currentId && buffer.isNotEmpty) {
                content.addAll(renderDays(buffer));
                if (currentId != null &&
                    shoppingLists != null &&
                    shoppingLists[currentId] != null) {
                  content.add(pw.SizedBox(height: 20));
                  content.add(pw.Text(strings.petMenuShoppingList.toUpperCase(),
                      style: pw.TextStyle(
                          fontWeight: pw.FontWeight.bold,
                          fontSize: 14,
                          color: PdfColors.black)));
                  ...
                  content.add(pw.TableHelper.fromTextArray(
                      ...
                      cellStyle: const pw.TextStyle(
                          fontSize: 9, color: PdfColors.black),
                      headers: [
                        strings.commonItem ?? 'Item',
                        strings.commonQuantity ?? 'Qtd',
                        strings.commonCategory ?? 'Info'
                      ],
                      data: list
                          .map((i) => [
                                i['item']?.toString() ??
            ...
            if (guidelines != null && guidelines.isNotEmpty) {
              ...
              content.add(pw.Container(
                ...
                child: pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    pw.Text('${strings.pdfFieldGeneralGuidelines}:',
                        style: pw.TextStyle(
                            fontWeight: pw.FontWeight.bold,
                            fontSize: 11,
                            color: PdfColors.black)),
              ...
              content.add(pw.Container(
                ...
                decoration: pw.BoxDecoration(
                  ...
                  border: pw.Border.all(
                      color: PdfColor.fromHex('#4CAF50'), width: 1.0),
                  borderRadius:
                    ...
                    ...recommendedBrands.map((item) {
                      ...
                      if (item is String) {
                        ...
                        reason = item['reason']?.toString() ??
                            item['por_que_escolhemos']?.toString();
                      } else {
                        // BrandSuggestion object
                        brandName = (item as dynamic).brand ??
                            ''; // Cast to dynamic or typed if possible
  ...
  Future<pw.Document> generateHumanNutritionPlanReport({
    ...
    final pdf = pw.Document();
    final String timestampStr =
        DateFormat('dd/MM/yyyy HH:mm', strings.localeName)
            .format(DateTime.now());
      ...
      try {
        ...
        if (decoded is List) {
          for (var item in decoded) {
            if (item is Map) {
              weeklyShoppingLists.add(
                  WeeklyShoppingList.fromJson(Map<String, dynamic>.from(item)));
            }
          }
        }
      } catch (e) {
    ...
    for (int w = 0; w < numWeeks; w++) {
      final startIdx = w * 7;
      final endIdx =
          (startIdx + 7) > days.length ? days.length : (startIdx + 7);
      ...
      pdf.addPage(
        pw.MultiPage(
          ...
          margin: const pw.EdgeInsets.all(35),
          header: (context) => buildHeader(
              '${strings.pdfMenuPlanTitle}${weekLabel.toUpperCase()}',
              timestampStr,
              color: orangeScanNut),
          footer: (context) => buildFooter(context, strings: strings),
          build: (context) => [
              ...
              child: pw.Column(
                ...
                children: [
                  pw.Row(
                    mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
                    children: [
                      pw.Text(strings.pdfPersonalizedPlanTitle,
                          style: pw.TextStyle(
                              fontWeight: pw.FontWeight.bold, fontSize: 12)),
                      pw.Text('${strings.pdfGoalLabel}: $goal',
                          style: pw.TextStyle(
                              fontWeight: pw.FontWeight.bold,
                              fontSize: 10,
                              color: orangeScanNut)),
                  ...
                  pw.SizedBox(height: 5),
                  pw.Text(strings.pdfGeneratedByLine,
                      style: const pw.TextStyle(
                          fontSize: 8, color: PdfColors.grey700)),
                ],
              ),
            ...
            // Detailed Days
            ...weekDays.map((day) {
              final String diaStr = DateFormat.MMMEd(strings.localeName)
                  .format(day.date)
                  .toUpperCase();
              ...
              return pw.Container(
                ...
                child: pw.Column(
                  ...
                  children: [
                    ...
                    ...day.meals.map((meal) {
                      return pw.Wrap(
                          // 🛡️ V80: Wrap to prevent infinite layout loop
                          children: [
                            pw.Container(
                              decoration: pw.BoxDecoration(
                                border: pw.Border(
                                    bottom: pw.BorderSide(
                                        color: orangeScanNut, width: 0.5)),
                              ),
                              padding: const pw.EdgeInsets.all(12),
                              child: pw.Column(
                                crossAxisAlignment: pw.CrossAxisAlignment.start,
                                children: [
                                  pw.Row(
                                    ...
                                    children: [
                                      pw.Text(
                                        _getMealLabel(meal.tipo, strings)
                                            .toUpperCase(),
                                        style: pw.TextStyle(
                                            fontWeight: pw.FontWeight.bold,
                                            fontSize: 10,
                                  ...
                                  pw.SizedBox(height: 10),
                                  pw.Text(strings.ingredientsTitle,
                                      style: pw.TextStyle(
                                          fontSize: 8,
                                          fontWeight: pw.FontWeight.bold,
            ...
            if (w == 0 &&
                batchCookingTips != null &&
                batchCookingTips.isNotEmpty) ...[
              pw.SizedBox(height: 10),
              pw.Container(
                ...
                child: pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    pw.Text(strings.pdfBatchCookingTips,
                        style: pw.TextStyle(
                            fontWeight: pw.FontWeight.bold,
                            fontSize: 11,
                            color: orangeScanNut)),
                    pw.SizedBox(height: 8),
                    pw.Text(batchCookingTips,
      ...
      if (w < weeklyShoppingLists.length) {
        ...
        pdf.addPage(
          pw.MultiPage(
            ...
            margin: const pw.EdgeInsets.all(35),
            header: (context) => buildHeader(
                '${strings.pdfShoppingListTitle}${weekLabel.toUpperCase()}',
                timestampStr,
                color: colorFood),
            footer: (context) => buildFooter(context, strings: strings),
            build: (context) => [
                ...
                return pw.Column(
                  ...
                  children: [
                    pw.Container(
                      ...
                      child: pw.Text(cat.title.toUpperCase(),
                          style: pw.TextStyle(
                              fontWeight: pw.FontWeight.bold,
                              fontSize: 11,
                              color: PdfColors.black)),
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          if (analysis.segurancaBiofilia
                      .segurancaDomestica['is_toxic_to_pets'] ==
                  true ||
              analysis.segurancaBiofilia
              ...
              analysis.segurancaBiofilia
                  ...
                  true ||
              analysis.segurancaBiofilia
                      .segurancaDomestica['is_toxic_to_children'] ==
                  true) ...[
            // Logic to determine Specific Toxicity Label
            (() {
              String label =
                  strings.toxicityWarning.toUpperCase(); // Default generic
              final sec = analysis.segurancaBiofilia.segurancaDomestica;
              final details =
                  (sec['toxicity_details'] ?? sec['sintomas_ingestao'] ?? '')
                      .toString()
                      .toLowerCase();
              final bool toxicKids = sec['toxica_para_criancas'] == true ||
                  sec['is_toxic_to_children'] == true;
              ...
              } else if (sec['toxica_para_pets'] == true ||
                  sec['is_toxic_to_pets'] == true ||
                  details.contains('animais') ||
                  details.contains('pets')) {
                    ...
                    pw.Expanded(
                      child: pw.Column(
                        ...
                        children: [
                          ...
                          pw.Text(
                              analysis.segurancaBiofilia
                                      .segurancaDomestica['toxicity_details'] ??
                                  analysis.segurancaBiofilia.segurancaDomestica[
                                      'sintomas_ingestao'] ??
                                  strings.plantDangerPets,
                              style: const pw.TextStyle(
            ...
            children: [
              ...
              pw.Expanded(
                child: pw.Column(
                  ...
                  children: [
                    pw.Text(
                        analysis.identificacao.nomesPopulares.isNotEmpty
                            ? analysis.identificacao.nomesPopulares.join(', ')
                            : analysis.identificacao.nomeCientifico,
                        style: pw.TextStyle(
                            fontWeight: pw.FontWeight.bold,
                            fontSize: 18,
                            color: PdfColors.green900)),
                    pw.Text(analysis.identificacao.nomeCientifico,
                        style: pw.TextStyle(
                            fontStyle: pw.FontStyle.italic,
                            fontSize: 14,
                    ...
                    pw.Row(
                      children: [
                        buildIndicator(
                            '${strings.tabHealth}:',
                            analysis.saude.condicao,
                            analysis.isHealthy
                                ? PdfColors.green700
                                : PdfColors.red700),
                        pw.SizedBox(width: 10),
                        buildIndicator('${strings.plantFamily}:',
                            analysis.identificacao.familia, PdfColors.black),
                      ],
                    ),
                  ],
                ),
          ...
          buildSectionHeader('1. ${strings.plantIdentificationTaxonomy}',
              color: colorPlant),
          pw.Text(
              '${strings.plantPopularNames}: ${analysis.identificacao.nomesPopulares.join(', ')}'),
          pw.Text(
              '${strings.plantScientificName}: ${analysis.identificacao.nomeCientifico}'),
          pw.Text('${strings.plantFamily}: ${analysis.identificacao.familia}'),
          pw.Text(
              '${strings.plantOrigin}: ${analysis.identificacao.origemGeografica}'),
          pw.SizedBox(height: 10),
          ...
          buildSectionHeader('2. ${strings.plantClinicalDiagnosis}',
              color: colorPlant),
          pw.Text(
              '${strings.plantClinicalDiagnosis}: ${analysis.saude.condicao}',
              style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
          pw.Text('${strings.plantDetails}: ${analysis.saude.detalhes}'),
          pw.Text(
              '${strings.plantRecoveryPlan}: ${analysis.saude.planoRecuperacao}'),
          if (!analysis.isHealthy)
            pw.Text('${strings.plantUrgency}: ${analysis.saude.urgencia}',
                style: pw.TextStyle(
                    color: PdfColors.red700, fontWeight: pw.FontWeight.bold)),
          pw.SizedBox(height: 10),

          // 3. GUIA DE SOBREVIVÊNCIA (HARDWARE)
          buildSectionHeader(
              '3. ${strings.tabHardware} (${strings.labelTrafficLight})',
              color: colorPlant),
            ...
            pw.Expanded(
                child: pw.Column(
                  ...
                  pw.Row(children: [
                    _buildCareIcon(
                        'sun',
                        _parsePlantCareLevel(
                            analysis.sobrevivencia.luminosidade['type'] ??
                                analysis.sobrevivencia.luminosidade['tipo'],
                            'sun')),
                    pw.SizedBox(width: 5),
                    pw.Text(strings.labelSun.toUpperCase(),
                        style: pw.TextStyle(
                            fontWeight: pw.FontWeight.bold, fontSize: 10)),
                  ]),
                  pw.SizedBox(height: 4),
                  pw.Text(analysis.sobrevivencia.luminosidade['type'] ??
                      analysis.sobrevivencia.luminosidade['tipo'] ??
                      strings.noInformation),
                  pw.Text(
                      analysis.sobrevivencia.luminosidade['explanation'] ??
                          analysis.sobrevivencia.luminosidade['explicacao'] ??
                          '',
                ...
                child: pw.Column(
                  ...
                  pw.Row(children: [
                    _buildCareIcon(
                        'water',
                        _parsePlantCareLevel(
                            analysis.sobrevivencia.regimeHidrico['frequency'] ??
                                analysis.sobrevivencia
                                    .regimeHidrico['frequencia_ideal'],
                            'water')),
                    pw.SizedBox(width: 5),
                    pw.Text(strings.labelWater.toUpperCase(),
                        style: pw.TextStyle(
                            fontWeight: pw.FontWeight.bold, fontSize: 10)),
                  ]),
                  pw.SizedBox(height: 4),
                  pw.Text(analysis.sobrevivencia.regimeHidrico['frequency'] ??
                      analysis
                          .sobrevivencia.regimeHidrico['frequencia_ideal'] ??
                      strings.noInformation),
                  pw.Text(
                      analysis.sobrevivencia.regimeHidrico['thirst_signs'] ??
                          analysis.sobrevivencia.regimeHidrico['sinais_sede'] ??
                          '',
                      style: const pw.TextStyle(fontSize: 8)),
                ])),
          ]),
          ...
          pw.Row(children: [
            _buildCareIcon(
                'soil',
                _parsePlantCareLevel(
                    analysis.sobrevivencia.soloENutricao['type'] ??
                        analysis.sobrevivencia.soloENutricao['tipo'],
                    'soil')),
            pw.SizedBox(width: 5),
            pw.Text(
                '${strings.labelSoil.toUpperCase() ?? strings.plantNeedSoil} & ${strings.tabNutrition}:',
                style:
                    pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10)),
          ]),
          pw.SizedBox(height: 4),
          pw.Text(
              '${strings.plantSubstrate}: ${analysis.sobrevivencia.soloENutricao['soil_type'] ?? analysis.sobrevivencia.soloENutricao['composicao_substrato'] ?? strings.noInformation}'),
          pw.Text(
              '${strings.plantFertilizer}: ${analysis.sobrevivencia.soloENutricao['fertilizer'] ?? analysis.sobrevivencia.soloENutricao['adubo_recomendado'] ?? strings.noInformation}'),
          if (analysis.sobrevivencia.soloENutricao['ideal_ph'] != null)
            pw.Text(
                '${strings.plantIdealPh}: ${analysis.sobrevivencia.soloENutricao['ideal_ph']}'),

          // 4. SEGURANÇA E BIOFILIA (BIOS)
          buildSectionHeader(
              '4. ${strings.tabBios} (${strings.plantHomeSafety})',
              color: colorPlant),
          pw.Row(children: [
            pw.Expanded(
                child: pw.Text(
                    '${strings.plantDangerPets}: ${(analysis.segurancaBiofilia.segurancaDomestica['is_toxic_to_pets'] == true || analysis.segurancaBiofilia.segurancaDomestica['toxica_para_pets'] == true) ? "${strings.commonYes} ⚠️" : "${strings.commonNo} ✅"}')),
            pw.Expanded(
                child: pw.Text(
                    '${strings.plantDangerKids}: ${(analysis.segurancaBiofilia.segurancaDomestica['is_toxic_to_children'] == true || analysis.segurancaBiofilia.segurancaDomestica['toxica_para_criancas'] == true) ? "${strings.commonYes} ⚠️" : "${strings.commonNo} ✅"}')),
          ]),
          pw.Text(
              '${strings.plantToxicityDetails}: ${analysis.segurancaBiofilia.segurancaDomestica['toxicity_details'] ?? analysis.segurancaBiofilia.segurancaDomestica['sintomas_ingestao'] ?? strings.plantNoAlerts}',
              style: const pw.TextStyle(fontSize: 8, color: PdfColors.red700)),
          pw.SizedBox(height: 10),
          pw.Text(strings.plantBioPower,
              style:
                  pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10)),
          pw.Text(
              '${strings.plantAirScore}: ${analysis.segurancaBiofilia.poderesBiofilicos['air_purification_score'] ?? analysis.segurancaBiofilia.poderesBiofilicos['purificacao_ar_score'] ?? 5}/10'),
          pw.Text(
              '${strings.plantWellness}: ${analysis.segurancaBiofilia.poderesBiofilicos['wellness_impact'] ?? analysis.segurancaBiofilia.poderesBiofilicos['impacto_bem_estar'] ?? strings.noInformation}'),

          // 5. ENGENHARIA DE PROPAGAÇÃO
          buildSectionHeader('5. ${strings.plantPropagationEngine}',
              color: colorPlant),
          pw.Text('${strings.plantMethod}: ${analysis.propagacao.metodo}',
              style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
          pw.Text(
              '${strings.plantDifficulty}: ${analysis.propagacao.dificuldade}'),
          pw.Text(
              '${strings.plantStepByStep}: ${analysis.propagacao.passoAPasso}'),

          // 6. INTELIGÊNCIA DE ECOSSISTEMA
          buildSectionHeader('6. ${strings.plantEcoIntel}', color: colorPlant),
          pw.Text(
              '${strings.plantCompanions}: ${analysis.ecossistema.plantasParceiras.join(", ")}'),
          pw.Text(
              '${strings.plantAvoid}: ${analysis.ecossistema.plantasConflitantes.join(", ")}'),
          pw.Text(
              '${strings.plantRepellent}: ${analysis.ecossistema.repelenteNatural}'),

          // 7. ESTÉTICA E LIFESTYLE (FENG SHUI)
          buildSectionHeader('7. ${strings.tabLifestyle}', color: colorPlant),
          pw.Text(
              '${strings.plantPlacement}: ${analysis.lifestyle.posicionamentoIdeal}'),
          pw.Text(
              '${strings.plantSymbolism}: ${analysis.lifestyle.simbolismo}'),
          pw.SizedBox(height: 10),
          pw.Text(strings.plantLivingAesthetic,
              style:
                  pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10)),
          pw.Text(
              '${strings.plantFlowering}: ${analysis.estetica.epocaFloracao}'),
          pw.Text(
              '${strings.plantFlowerColor}: ${analysis.estetica.corDasFlores}'),
          pw.Text(
              '${strings.plantGrowth}: ${analysis.estetica.velocidadeCrescimento}'),
          pw.Text(
              '${strings.plantMaxSize}: ${analysis.estetica.tamanhoMaximo}'),
  ...
  pw.Widget _buildCareLegend(AppLocalizations strings) {
    return pw.Container(
      ...
      child: pw.Column(
        crossAxisAlignment: pw.CrossAxisAlignment.start,
        children: [
          pw.Text(strings.pdfCareLegendTitle,
              style: pw.TextStyle(
                  fontWeight: pw.FontWeight.bold,
                  fontSize: 10,
                  color: PdfColors.green900)),
          ...
          pw.Row(
            ...
            children: [
              _buildLegendItem(
                  'sun',
                  strings.plantNeedSun,
                  'Nível de intensidade de luz solar necessária.',
                  PdfColors.orange),
              _buildLegendItem('water', strings.plantNeedWater,
                  'Frequência de hidratação do solo.', PdfColors.blue),
              _buildLegendItem('soil', strings.plantNeedSoil,
                  'Riqueza e drenagem do substrato.', PdfColors.brown),
            ],
          ),
          pw.SizedBox(height: 10),
          pw.Divider(color: PdfColors.grey300, thickness: 0.5),
          pw.SizedBox(height: 5),
          pw.Text(strings.pdfCareLegendLevels,
              style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 8)),
          pw.Text(strings.pdfCareLegendDescription,
              style: const pw.TextStyle(fontSize: 8)),
        ],
      ),
    );
  ...
  pw.Widget _buildLegendItem(
      String type, String title, String desc, PdfColor color) {
    return pw.Expanded(
      child: pw.Row(
        children: [
  ...
  String _translateKey(String key) {
    ...
    if (_keyTranslations.containsKey(withUnderscore)) {
      return _keyTranslations[withUnderscore]!;
    }

    final withSpace = upper.replaceAll('_', ' ');
    if (_keyTranslations.containsKey(withSpace)) {
      return _keyTranslations[withSpace]!;
    }
  ...
  String _translateSex(String? val, AppLocalizations str) {
    ...
    if (v.contains('fêmea') || v.contains('femea') || v.contains('female')) {
      return 'Fêmea';
    }
  ...
  Iterable<pw.Widget> _buildRecursivePDFMap(Map<dynamic, dynamic> map,
      {double indent = 0}) sync* {
        ...
        yield pw.Padding(
            ...
            child: pw.Text(_translateKey(key),
                style: pw.TextStyle(
                    fontWeight: pw.FontWeight.bold,
                    fontSize: 10,
                    color: PdfColors.black) // V64: Consistent Black Titles
                ));
        yield pw.Divider(
  ...
  Iterable<pw.Widget> _buildAnalysisResultSection(
      Map<String, dynamic> data, AppLocalizations strings) sync* {
    // Filter top-level
    ...
    yield buildSectionHeader(strings.petAnalysisResults.toUpperCase(),
        color: colorPet);
  ...
  Future<pw.Document> generatePetProfileReport({
    ...
    try {
      ...
      debugPrint(
          '📅 Loaded ${allAgendaEvents.length} total agenda events, ${medicalEvents.length} medical events');
    } catch (e) {
    ...
    if (sections['gallery'] == true) {
      try {
        ...
        final allDocs =
            await FileUploadService().getMedicalDocuments(profile.petName);
        ...
        final Map<String, File> uniqueImages =
            {}; // Key can be timestamp OR file size hash
    ...
    final DateTime? savedStart =
        profile.rawAnalysis?['data_inicio_semana'] != null
    ...
    if (savedStart != null) {
      ...
      startData = DateTime(baseDate.year, baseDate.month, baseDate.day)
    ...
    if (sections['health'] == true) {
      ...
      if (profile.historicoAnaliseFeridas.isNotEmpty) {
        final sorted =
            List<AnaliseFeridaModel>.from(profile.historicoAnaliseFeridas)
      ...
      // Source 2: Legacy History (Fallback)
        ...
        final sortedWounds =
            List<Map<String, dynamic>>.from(profile.woundAnalysisHistory)
              ..sort((a, b) {
                try {
        ...
        for (var w in sortedWounds) {
          ...
          woundsWithImages.add({...w, 'pdfImage': img});
        }
      }
    }
    ...
    if (sections['parc'] == true && profile.linkedPartnerIds.isNotEmpty) {
      try {
        debugPrint(
            '👥 Loading partner data for ${profile.linkedPartnerIds.length} partners');
        ...
        debugPrint(
            '👥 Partner loading complete: ${linkedPartnersData.length} partners loaded');
      } catch (e) {
            ...
            children: [
              ...
              if (profileImage != null)
                pw.Container(
                  ...
                  decoration: pw.BoxDecoration(
                    border: pw.Border.all(
                        color: PdfColors.black, width: 4), // Black border
                    borderRadius:
                        const pw.BorderRadius.all(pw.Radius.circular(100)),
                  ),
                  child: pw.ClipRRect(
              ...
              pw.Container(
                ...
                decoration: pw.BoxDecoration(
                  color: PdfColors.white,
                  border: pw.Border.all(
                      color: PdfColors.black, width: 2), // Black border
                  borderRadius:
    ...
    final headerStyle = pw.TextStyle(
        color: PdfColors.black, fontWeight: pw.FontWeight.bold, fontSize: 10);
    ...
    final growthData = (profile.rawAnalysis?['identificacao'] != null &&
            profile.rawAnalysis!['identificacao']['curva_crescimento'] != null)
        ? (profile.rawAnalysis!['identificacao']['curva_crescimento'] as Map)
        : {};
    ...
    if (profile.labExams.isNotEmpty) {
      ...
      labExamsWidgets.add(pw.Text('${strings.pdfExamesLab}:',
          style: pw.TextStyle(
              fontSize: 11,
              fontWeight: pw.FontWeight.bold,
              color: PdfColors.black)));
      ...
      for (var examJson in profile.labExams) {
        ...
        switch (exam.category.toLowerCase()) {
          case 'blood':
            categoryLabel = 'Exame de Sangue';
            break;
          case 'urine':
            categoryLabel = 'Exame de Urina';
            break;
          case 'feces':
            categoryLabel = 'Exame de Fezes';
            break;
          case 'other':
            categoryLabel = 'Outro Exame';
            break;
        }

        // Header
        labExamsWidgets.add(pw.Container(
          padding: const pw.EdgeInsets.symmetric(vertical: 4, horizontal: 8),
          decoration: const pw.BoxDecoration(
              color: PdfColors.grey100,
              border: pw.Border(
          ...
          child: pw.Row(
            mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
            children: [
              pw.Text(categoryLabel.toUpperCase(),
                  style: pw.TextStyle(
                      fontSize: 10,
                      fontWeight: pw.FontWeight.bold,
                      color: PdfColors.black)),
        ...
        if (exam.aiExplanation != null && exam.aiExplanation!.isNotEmpty) {
          ...
          labExamsWidgets.add(pw.Text('ANÁLISE DA IA:',
              style: pw.TextStyle(
                  fontSize: 8,
                  fontWeight: pw.FontWeight.bold,
          ...
          for (var line in lines) {
            ...
            labExamsWidgets.add(pw.Padding(
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        margin: const pw.EdgeInsets.all(35),
        header: (context) => buildHeader(
            '${strings.pdfReportTitle}: ${profile.petName}', timestampStr,
            dateLabel: strings.pdfGeneratedOn, color: colorPet),
        footer: (context) => buildFooter(context),
        build: (context) => [
          // ========== IDENTITY SECTION ==========
          if (sections['identity'] == true) ...[
                ...
                pw.TableRow(
                  ...
                  children: [
                    pw.Padding(
                        padding: padding,
                        child:
                            pw.Text(strings.pdfFieldLabel, style: headerStyle)),
                    pw.Padding(
                        padding: padding,
                        child:
                            pw.Text(strings.pdfFieldValue, style: headerStyle)),
                  ],
                ),
                // Pet Name (Bold!)
                pw.TableRow(
                  children: [
                    pw.Padding(
                        padding: padding,
                        child: pw.Text(strings.pdfFieldName, style: cellStyle)),
                    pw.Padding(
                        padding: padding,
                        child: pw.Text(profile.petName, style: boldCellStyle)),
                  ],
                ),
                // Other Standard Rows
                for (var row in [
                  [
                    strings.pdfFieldBreed,
                    profile.raca ?? strings.petNotIdentified
                  ],
                  [
                    strings.pdfFieldAge,
                    profile.idadeExata ?? strings.petNotOffice
                  ],
                  [
                    strings.pdfFieldSex,
                    _translateSex(
                        profile.sex ??
                            profile.rawAnalysis?['identificacao']?['sexo'],
                        strings)
                  ],
                  [
                    strings.pdfFieldMicrochip,
                    profile.rawAnalysis?['identificacao']?['microchip'] ??
                        strings.petNotOffice
                  ],
                  [
                    strings.pdfFieldCurrentWeight,
                    profile.pesoAtual != null
                        ? '${profile.pesoAtual} kg'
                        : strings.petNotOffice
                  ],
                  [strings.pdfFieldIdealWeight, pesoIdealDisplay],
                  [
                    strings.pdfFieldReproductiveStatus,
                    profile.statusReprodutivo ?? strings.petNotOffice
                  ],
                  [
                    strings.pdfFieldActivityLevel,
                    profile.nivelAtividade ?? strings.petActivityModerate
                  ],
                  [
                    strings.pdfFieldBathFrequency,
                    profile.frequenciaBanho ?? strings.petNotOffice
                  ],
                  [
                    'Porte',
                    profile.porte ?? strings.petNotIdentified
                  ], // 🛡️ V128: Add Porte
                ])
                  pw.TableRow(children: [
                    pw.Padding(
                        padding: padding,
                        child: pw.Text(row[0].toString(), style: cellStyle)),
                    pw.Padding(
                        padding: padding,
                        child: pw.Text(row[1].toString(), style: cellStyle)),
                  ])
              ],
            ),
            ...
            if (growthData.isNotEmpty) ...[
              pw.SizedBox(height: 15),
              pw.Text('${strings.petGrowthCurve}:',
                  style: pw.TextStyle(
                      fontSize: 10,
                      fontWeight: pw.FontWeight.bold,
                      color: PdfColors.black)),
                  ...
                  children: [
                    pw.TableRow(
                        ...
                        children: [
                          pw.Padding(
                              padding: padding,
                              child: pw.Text('3 Meses', style: headerStyle)),
                          pw.Padding(
                              padding: padding,
                              child: pw.Text('6 Meses', style: headerStyle)),
                          pw.Padding(
                              padding: padding,
                              child: pw.Text('12 Meses', style: headerStyle)),
                          pw.Padding(
                              padding: padding,
                              child:
                                  pw.Text('Adulto (Est.)', style: headerStyle)),
                        ]),
                    pw.TableRow(children: [
                      pw.Padding(
                          padding: padding,
                          child: pw.Text(
                              growthData['peso_3_meses']?.toString() ?? '-',
                              style: cellStyle)),
                      pw.Padding(
                          padding: padding,
                          child: pw.Text(
                              growthData['peso_6_meses']?.toString() ?? '-',
                              style: cellStyle)),
                      pw.Padding(
                          padding: padding,
                          child: pw.Text(
                              growthData['peso_12_meses']?.toString() ?? '-',
                              style: cellStyle)),
                      pw.Padding(
                          padding: padding,
                          child: pw.Text(
                              growthData['peso_adulto']?.toString() ?? '-',
                              style: cellStyle)),
                    ])
                  ])
            ],
            ...
            if (profile.restricoes.isNotEmpty) ...[
              ...
              pw.Container(
                ...
                child: pw.Text(
                  ...
                  style:
                      const pw.TextStyle(fontSize: 9, color: PdfColors.red900),
                ),
              ),
            ],

            // Análise da Raça e Perfil (Dados Estendidos)
            if (profile.analysisHistory.isNotEmpty ||
                profile.rawAnalysis != null) ...[
              ..._buildAnalysisResultSection(
                  profile.analysisHistory.isNotEmpty
                      ? profile.analysisHistory.last
                      : profile.rawAnalysis!,
                  strings),
            ],
          ...
          if (sections['health'] == true) ...[
              ...
              data: [
                [
                  ...
                  profile.pesoAtual != null && profile.pesoIdeal != null
                      ? (profile.pesoAtual! > profile.pesoIdeal!
                          ? strings.pdfPesoStatusOver
                          : profile.pesoAtual! < profile.pesoIdeal!
                              ? strings.pdfPesoStatusUnder
                              : strings.pdfPesoStatusIdeal)
                      : 'N/A'
                ],
                [
                  strings.pdfFieldIdealWeight,
                  profile.pesoIdeal != null
                      ? '${profile.pesoIdeal} kg'
                      : strings.petNotOffice,
            ...
            if (profile.weightHistory.isNotEmpty) ...[
              ...
              pw.TableHelper.fromTextArray(
                ...
                headers: [
                  strings.pdfDate,
                  '${strings.pdfFieldCurrentWeight} (kg)',
                  strings.pdfStatus
                ],
                data: profile.weightHistory
                    .take(10)
                    .map((entry) => [
                          DateFormat.yMd(strings.localeName).format(
                              DateTime.tryParse(
                                      entry['date']?.toString() ?? '') ??
                                  DateTime.now()),
                          '${entry['weight']} kg',
            ...
            // Vacinas
            pw.Text(
                '${strings.pdfVacinaV10} & ${strings.pdfVacinaAntirrabica}:',
                style: pw.TextStyle(
                    fontSize: 11,
                    fontWeight: pw.FontWeight.bold,
                    color: PdfColors.black)),
              ...
              data: [
                [
                  ...
                  profile.dataUltimaV10 != null
                      ? DateFormat.yMd(strings.localeName)
                          .format(profile.dataUltimaV10!)
                      : strings.pdfVacinaNaoRegistrada,
                  profile.dataUltimaV10 != null
                      ? DateFormat.yMd(strings.localeName).format(
                          profile.dataUltimaV10!.add(const Duration(days: 365)))
                      : 'N/A',
                ],
                [
                  ...
                  profile.dataUltimaAntirrabica != null
                      ? DateFormat.yMd(strings.localeName)
                          .format(profile.dataUltimaAntirrabica!)
                      : strings.pdfVacinaNaoRegistrada,
                  profile.dataUltimaAntirrabica != null
                      ? DateFormat.yMd(strings.localeName).format(profile
            ...
            pw.Container(
              ...
              child: pw.Text(
                profile.alergiasConhecidas.isEmpty
                    ...
                    : strings.pdfAlergiasAviso(
                        profile.alergiasConhecidas.join(', ')),
                style: pw.TextStyle(
                  fontSize: 9,
                  fontWeight: pw.FontWeight.bold,
                  color: PdfColors.black,
            ...
            if (profile.analysisHistory.any((a) =>
                (a['clinical_signs'] ?? a['clinicalSigns']) != null)) ...[
              pw.SizedBox(height: 15),
              ...
              ...profile.analysisHistory
                ...
                final dateStr = DateFormat.yMd(strings.localeName).format(
                    DateTime.tryParse(analysis['last_updated']?.toString() ??
                            analysis['created_at']?.toString() ??
                            '') ??
                        DateTime.now());

                final signs = (analysis['clinical_signs'] ??
                    analysis['clinicalSigns'] ??
                    {}) as Map;
                ...
                if (analysis['possiveis_causas'] != null) {
                  if (analysis['possiveis_causas'] is List) {
                    diagnosis =
                        (analysis['possiveis_causas'] as List).join(', ');
                  } else {
                                ...
                                if (signs['eyes'] != null)
                                  pw.Expanded(
                                      child: pw.Column(
                                          ...
                                          children: [
                                        pw.Text(strings.pdfEyes,
                                            style: pw.TextStyle(
                                                fontWeight: pw.FontWeight.bold,
                                                fontSize: 8)),
                                        pw.Text(signs['eyes'].toString(),
                                            style: const pw.TextStyle(
                                                fontSize: 8)),
                                      ])),
                                if (signs['eyes'] != null)
                                  ...
                                  pw.Expanded(
                                      child: pw.Column(
                                          crossAxisAlignment:
                                              pw.CrossAxisAlignment.start,
                                          children: [
                                        pw.Text(strings.pdfTeeth,
                                            style: pw.TextStyle(
                                                fontWeight: pw.FontWeight.bold,
                                                fontSize: 8)),
                                        pw.Text(
                                            (signs['dental'] ?? signs['teeth'])
                                                .toString(),
                                            style: const pw.TextStyle(
                                                fontSize: 8)),
                                      ])),
                                  ...
                                  pw.Expanded(
                                      child: pw.Column(
                                          crossAxisAlignment:
                                              pw.CrossAxisAlignment.start,
                                          children: [
                                        pw.Text(strings.pdfSkin,
                                            style: pw.TextStyle(
                                                fontWeight: pw.FontWeight.bold,
                                                fontSize: 8)),
                                        pw.Text(signs['skin'].toString(),
                                            style: const pw.TextStyle(
                                                fontSize: 8)),
                                      ])),
                              ])
            ...
            if (woundsWithImages.isNotEmpty) ...[
              ...
              ...woundsWithImages.map((analysis) {
                final dateStr = DateFormat.yMd(strings.localeName)
                    .add_Hm()
                    .format(
                        DateTime.tryParse(analysis['date']?.toString() ?? '') ??
                            DateTime.now());
                final severity = analysis['severity'] ?? 'N/A';
                final diagnosis =
                    analysis['diagnosis'] ?? strings.petDiagnosisDefault;
                      ...
                      pw.Expanded(
                        child: pw.Column(
                          ...
                          children: [
                            ...
                            pw.SizedBox(height: 4),
                            pw.Text(strings.pdfDiagnosis(diagnosis),
                                style: const pw.TextStyle(fontSize: 9)),
                            if (recommendations.isNotEmpty) ...[
                              pw.SizedBox(height: 4),
                              pw.Text('${strings.pdfRecommendations}:',
                                  style: pw.TextStyle(
                                      fontSize: 9,
                                      fontWeight: pw.FontWeight.bold)),
          ...
          if (sections['nutrition'] == true) ...[
              ...
              pw.Container(
                ...
                child: pw.Row(
                  ...
                  children: [
                    ...
                    pw.Text(
                      profile.rawAnalysis!['tipo_dieta']?.toString() ??
                          strings.petNotOffice,
            ...
            if (profile.rawAnalysis != null &&
                profile.rawAnalysis!['plano_semanal'] != null) ...[
              ...
              ...(profile.rawAnalysis!['plano_semanal'] as List)
                  .asMap()
                  .entries
                  .expand((entry) {
                try {
                  ...
                  final weekDayName =
                      DateFormat('EEEE', strings.localeName).format(dateForDay);
                  final weekDayCap =
                  ...
                  debugPrint(
                      '⚠️ [V72-PDF] Error rendering meal plan day ${entry.key}: $e');
                  return <pw.Widget>[];
                }
              }),
            ] else ...[
          ...
          if (sections['gallery'] == true) ...[
            ...
            if (otherDocNames.isNotEmpty) ...[
              pw.SizedBox(height: 15),
              pw.Text(strings.pdfAttachedDocs,
                  style: pw.TextStyle(
                      fontWeight: pw.FontWeight.bold,
                      fontSize: 10,
                      color: PdfColors.black)),
            ...
            if (profile.linkedPartnerIds.isNotEmpty) ...[
              ...
              if (linkedPartnersData.isNotEmpty) ...[
                  ...
                  return pw.Container(
                    ...
                    child: pw.Column(
                      ...
                      children: [
                        ...
                        if (partnerData['specialties'] != null &&
                        ...
                        if (partnerData['phone'] != null &&
                            partnerData['phone'].toString().isNotEmpty) ...[
                          pw.SizedBox(height: 4),
                          pw.Row(
                            children: [
                              pw.Text('📞 ',
                                  style: const pw.TextStyle(fontSize: 8)),
                              pw.Text(partnerData['phone'],
                                  style: const pw.TextStyle(fontSize: 8)),
                            ],
                          ),
                        ],
                        if (partnerData['address'] != null &&
                            partnerData['address'].toString().isNotEmpty) ...[
                          pw.SizedBox(height: 2),
                          pw.Row(
                            children: [
                              pw.Text('📍 ',
                                  style: const pw.TextStyle(fontSize: 8)),
                              pw.Expanded(
                                child: pw.Text(partnerData['address'],
                                    style: const pw.TextStyle(fontSize: 8)),
                              ),
                            ],
                          ),
                ...
                pw.Container(
                  ...
                  child: pw.Text(
                    strings
                        .pdfPartnerLoadError(profile.linkedPartnerIds.length),
                    style: const pw.TextStyle(fontSize: 9),
                  ),
                ),
              ],
          ...
          if (allAgendaEvents.isNotEmpty) ...[
            ...
            ...() {
              ...
              final upcomingEvents = allAgendaEvents
                  .where((e) => e.dateTime.isAfter(now))
                  .toList();
              final pastEvents = allAgendaEvents
                  .where((e) => !e.dateTime.isAfter(now))
                  .toList();
              ...
              return [
                ...
                if (upcomingEvents.isNotEmpty) ...[
                  ...
                  pw.SizedBox(height: 6),
                  ...upcomingEvents.take(10).map((event) =>
                      _buildEventItem(event, strings, isUpcoming: true)),
                  pw.SizedBox(height: 12),
                ],
                ...
                if (pastEvents.isNotEmpty) ...[
                  ...
                  pw.SizedBox(height: 6),
                  ...pastEvents.take(15).map((event) =>
                      _buildEventItem(event, strings, isUpcoming: false)),
                ],
              ];
            }(),
  ...
  Future<pw.Document> generatePartnersHubReport({
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        margin: const pw.EdgeInsets.all(35),
        header: (context) => buildHeader(
            '${strings.pdfReportTitle}: ${strings.pdfParcSection}',
            timestampStr,
            dateLabel: strings.pdfGeneratedOn,
            color: colorPet),
        footer: (context) => buildFooter(context),
        build: (context) => [
          // INDICADORES
          pw.Row(
            ...
            children: [
              buildIndicator(
                  strings.partnersTitle, partners.length.toString(), colorPet),
              ...counts.entries.take(2).map((e) =>
                  buildIndicator(e.key, e.value.toString(), colorPetLight)),
            ],
          ),
          if (counts.length > 2) ...[
            pw.SizedBox(height: 10),
              ...
              children: counts.entries
                  ...
                  .map((e) => pw.Padding(
                        ...
                        child: buildIndicator(
                            e.key, e.value.toString(), PdfColors.black),
                      ))
                  .toList(),
            ),
          ],
          ...
          pw.TableHelper.fromTextArray(
            ...
            headerStyle: pw.TextStyle(
                color: PdfColors.black,
                fontWeight: pw.FontWeight.bold,
                fontSize: 9), // Black text
            cellStyle: const pw.TextStyle(fontSize: 8),
            headers: reportType == strings.partnersSummary
                ? [
                    strings.pdfFieldName,
                    strings.partnersCategory,
                    strings.pdfFieldPhone
                  ]
                : [
                    strings.pdfFieldName,
                    strings.partnersCategory,
                    strings.pdfFieldPhone,
                    strings.pdfFieldAddress,
                    strings.pdfFieldEmail,
                    strings.pdfFieldObservations
                  ],
  ...
  Future<pw.Document> generateSinglePartnerReport({
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        margin: const pw.EdgeInsets.all(35),
        header: (context) => buildHeader(
            '${strings.pdfReportTitle}: ${partner.name}', timestampStr,
            dateLabel: strings.pdfGeneratedOn, color: colorPet),
        footer: (context) => buildFooter(context),
        build: (context) => [
          // HEADER INFO
          pw.Row(
              ...
              children: [
                buildIndicator(
                    strings.partnersCategory, partner.category, colorPet),
                if (partner.rating > 0)
                  buildIndicator(strings.pdfRating,
                      '${partner.rating} ${strings.pdfStars}', colorPetLight),
              ]),
          pw.SizedBox(height: 20),
          ...
          pw.Table(columnWidths: {
            ...
            if (partner.cnpj != null && partner.cnpj!.isNotEmpty)
              _buildTableRow('CNPJ:', partner.cnpj!),
            _buildTableRow(strings.pdfFieldPhone, partner.phone),
            if (partner.whatsapp != null &&
                partner.whatsapp!.isNotEmpty &&
                partner.whatsapp != partner.phone)
              _buildTableRow('WhatsApp:', partner.whatsapp!),
            if (partner.email != null && partner.email!.isNotEmpty)
              _buildTableRow(strings.pdfFieldEmail, partner.email!),
            if (partner.website != null && partner.website!.isNotEmpty)
              _buildTableRow('Website:', partner.website!),
            if (partner.instagram != null && partner.instagram!.isNotEmpty)
              _buildTableRow('Instagram:', partner.instagram!),
            _buildTableRow(strings.pdfFieldAddress, partner.address),
            if (partner.openingHours['raw'] != null &&
                partner.openingHours['raw'].toString().isNotEmpty)
              _buildTableRow(
                  strings.partnerFieldHours, partner.openingHours['raw']),
          ]),
          pw.SizedBox(height: 15),
          if (partner.openingHours['plantao24h'] == true)
            pw.Row(children: [
              buildIndicator(
                  strings.partnerField24h, 'ATIVO / ACTIVE', PdfColors.red700),
            ]),
          pw.SizedBox(height: 20),
          ...
          if (notes.isNotEmpty) ...[
            ...
            ...notes.map((n) {
              ...
              final formattedDate =
                  DateFormat.yMd(strings.localeName).add_Hm().format(date);
              return pw.Padding(
          ...
          if (partner.specialties.isNotEmpty) ...[
            ...
            pw.SizedBox(height: 10),
            pw.Text(partner.specialties.join(', '),
                style: const pw.TextStyle(fontSize: 10)),
          ]
        ],
      ),
  ...
  pw.Widget _buildEventItem(PetEvent event, AppLocalizations strings,
    ...
    final dateStr =
        DateFormat.yMd(strings.localeName).add_Hm().format(event.dateTime);
  ...
  String _localizeCategory(String category, AppLocalizations strings) {
    ...
    if (c.contains('farm') || c.contains('pharm')) {
      return strings.partnersFilterPharmacy;
    }
    if (c.contains('shop') || c.contains('tienda')) {
      return strings.partnersFilterPetShop;
    }
    if (c.contains('banho') || c.contains('grooming') || c.contains('peluquer')) {
      return strings.partnersFilterGrooming;
    }
    if (c.contains('hotel')) return strings.partnersFilterHotel;
    if (c.contains('lab') ||
        c.contains('laboratório') ||
        c.contains('laboratory')) {
      return strings.partnersFilterLab;
    }
    return category;
  }
    ...
    for (var item in items) {
      ...
      if (item.rawMetadata != null) {
        try {
          ...
          debugPrint(
              '⚠️ Error parsing history item ${item.foodName} for PDF: $e');
        }
      }
      ...
      pdf.addPage(
        pw.MultiPage(
          ...
          margin: const pw.EdgeInsets.all(35),
          header: (context) => buildHeader(
              "${strings.pdfFoodTitle} - ${item.foodName}", timestampStr,
    ...
    for (var item in items) {
      pdf.addPage(
        pw.MultiPage(
          ...
          margin: const pw.EdgeInsets.all(35),
          header: (context) => buildHeader(
              "RECEITA - ${item.recipeName.toUpperCase()}", timestampStr,
              dateLabel: strings.pdfDate, color: colorFood),
          footer: (context) => buildFooter(context, strings: strings),
          build: (context) => [
            pw.Center(
              child: pw.Text(item.recipeName.toUpperCase(),
                  style: pw.TextStyle(
                      fontWeight: pw.FontWeight.bold,
                      fontSize: 18,
                      color: PdfColors.black),
            ...
            pw.Container(
              ...
              child: pw.Text(
                item.instructions,
                style: const pw.TextStyle(
                    fontSize: 10, height: 1.5, color: PdfColors.black),
              ),
            ),
            pw.SizedBox(height: 20),
          ],
  ...
  Future<pw.Document> generatePlantHistoryReport({
    ...
    for (var item in items) {
      ...
      String toxicityLabel = item.toxicityStatus == 'harmful_pets'
          ? 'TÓXICA: ANIMAIS'
      ...
      if (isToxic) {
        ...
        if (meta != null && meta['seguranca_biofilia'] != null) {
          ...
          if (sec != null) {
            final details =
                (sec['sintomas_ingestao'] ?? sec['toxicity_details'] ?? '')
                    .toString()
                    .toLowerCase();
            final bool toxicPets = sec['toxica_para_pets'] == true ||
                sec['is_toxic_to_pets'] == true;
            final bool toxicKids = sec['toxica_para_criancas'] == true ||
                sec['is_toxic_to_children'] == true;
      ...
      pdf.addPage(
        pw.MultiPage(
          ...
          margin: const pw.EdgeInsets.all(35),
          header: (context) => buildHeader(
              "PLANTA - ${item.plantName.toUpperCase()}", timestampStr,
              dateLabel: strings.pdfDate, color: colorPlant),
          footer: (context) => buildFooter(context, strings: strings),
          build: (context) => [
            // Plant Name
            pw.Center(
              child: pw.Text(item.plantName.toUpperCase(),
                  style: pw.TextStyle(
                      fontWeight: pw.FontWeight.bold,
                      fontSize: 18,
                      color: PdfColors.black),
              ...
              pw.Container(
                ...
                child: pw.Column(
                  ...
                  children: [
                    ...
                    pw.Text(_getBotanyHistoryToxicityText(item),
                        style: const pw.TextStyle(
                            fontSize: 10, height: 1.5, color: PdfColors.black)),
                  ],
                ),
                      ...
                      child: pw.Row(
                        children: [
                          _buildCareIcon(
                              'sun',
                              _parsePlantCareLevel(
                                  item.lightWaterSoilNeeds['luz'], 'sun'),
                              size: 10),
                          pw.SizedBox(width: 5),
                          pw.Text('${strings.labelSun}: ',
                              style: pw.TextStyle(
                                  fontWeight: pw.FontWeight.bold,
                                  fontSize: 10)),
                          pw.Expanded(
                              child: pw.Text(item.lightWaterSoilNeeds['luz']!,
                                  style: const pw.TextStyle(fontSize: 10))),
                        ],
                      ),
                    ),
                      ...
                      child: pw.Row(
                        children: [
                          _buildCareIcon(
                              'water',
                              _parsePlantCareLevel(
                                  item.lightWaterSoilNeeds['agua'], 'water'),
                              size: 10),
                          pw.SizedBox(width: 5),
                          pw.Text('${strings.labelWater}: ',
                              style: pw.TextStyle(
                                  fontWeight: pw.FontWeight.bold,
                                  fontSize: 10)),
                          pw.Expanded(
                              child: pw.Text(item.lightWaterSoilNeeds['agua']!,
                                  style: const pw.TextStyle(fontSize: 10))),
                        ],
                      ),
                    ),
                    ...
                    pw.Row(
                      children: [
                        _buildCareIcon(
                            'soil',
                            _parsePlantCareLevel(
                                item.lightWaterSoilNeeds['solo'], 'soil'),
                            size: 10),
                        pw.SizedBox(width: 5),
                        pw.Text('${strings.labelSoil}: ',
                            style: pw.TextStyle(
                                fontWeight: pw.FontWeight.bold, fontSize: 10)),
                        pw.Expanded(
                            child: pw.Text(item.lightWaterSoilNeeds['solo']!,
                                style: const pw.TextStyle(fontSize: 10))),
                      ],
                    ),
                ],
  ...
  Future<pw.Document> generateVeterinary360Report({
    ...
    debugPrint(
        '[PDF-DEBUG] Starting History Merge in Vet360. Profile: ${profile?.petName}');
    List<AnaliseFeridaModel> fullHistory = [];
    if (profile != null) {
      debugPrint(
          '[PDF-DEBUG] Raw Unified History Count: ${profile.historicoAnaliseFeridas.length}');
      fullHistory.addAll(profile.historicoAnaliseFeridas);

      // Merge Legacy
      debugPrint(
          '[PDF-DEBUG] Raw Legacy History Count: ${profile.woundAnalysisHistory.length}');
      if (profile.woundAnalysisHistory.isNotEmpty) {
        for (var w in profile.woundAnalysisHistory) {
          try {
            fullHistory.add(AnaliseFeridaModel(
                dataAnalise: DateTime.tryParse(w['date']?.toString() ?? '') ??
                    DateTime.now(),
                ...
                diagnosticosProvaveis:
                    w['diagnosis'] != null ? [w['diagnosis'].toString()] : [],
                categoria: 'Historico'));
          } catch (e) {
      ...
      debugPrint(
          '[PDF-DEBUG] Total Merged History Items: ${fullHistory.length}');
    }
    ...
    if (fullHistory.isNotEmpty) {
      for (int i = 0; i < fullHistory.length; i++) {
        ...
        debugPrint(
            '[PDF-DEBUG] Processing Item $i: Date=${item.dataAnalise}, Ref="${item.imagemRef}"');

        if (item.imagemRef.isNotEmpty) {
          final img = await safeLoadImage(item.imagemRef);
          debugPrint(
              '[PDF-DEBUG] Image Load $i: ${img != null ? "SUCCESS" : "FAIL (null returned)"}');
          if (img != null) {
            historyImages[i] = img;
          }
        } else {
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        margin: const pw.EdgeInsets.all(35),
        header: (context) => buildHeader(
            '${strings.petDossierTitle.toUpperCase()} - $petName', timestampStr,
            color: colorPet),
        footer: (context) => buildFooter(context, strings: strings),
        build: (context) => [
                    ...
                    pw.Container(
                      ...
                      child: pw.Row(
                        ...
                        children: [
                          ...
                          pw.SizedBox(width: 5),
                          pw.Text(
                              '${strings.pdfPrecision.toUpperCase() ?? "PRECISÃO"}: $reliability',
                              style: pw.TextStyle(
                                  fontWeight: pw.FontWeight.bold,
                                  fontSize: 10,
                                  color: PdfColors.black)),
          ...
          // Identity Details
          pw.Text('1. ${strings.petSectionIdentity.toUpperCase()}',
              style:
                  pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 11)),
          pw.SizedBox(height: 5),
          pw.Bullet(
              text:
                  '${strings.pdfFieldBreed}: ${analysis.identificacao.racaPredominante}'),
          pw.Bullet(
              text: 'Linhagem: ${analysis.identificacao.linhagemSrdProvavel}'),
          // V470: Morphology & Origin (Re-enabled)
          if (analysis.identificacao.origemGeografica.trim().isNotEmpty &&
              analysis.identificacao.origemGeografica != 'N/A')
            pw.Bullet(
                text:
                    '${strings.petOriginRegion}: ${analysis.identificacao.origemGeografica}'),
          if (analysis.identificacao.morfologiaBase.trim().isNotEmpty &&
              analysis.identificacao.morfologiaBase != 'N/A')
            pw.Bullet(
                text:
                    '${strings.petMorphology}: ${analysis.identificacao.morfologiaBase}'),
                ...
                children: [
                  pw.TableRow(
                      ...
                      children: [
                        pw.Padding(
                            ...
                            child: pw.Text('3 Meses',
                                style: pw.TextStyle(
                                    fontWeight: pw.FontWeight.bold,
                                    fontSize: 8))),
                        pw.Padding(
                            padding: const pw.EdgeInsets.all(4),
                            child: pw.Text('6 Meses',
                                style: pw.TextStyle(
                                    fontWeight: pw.FontWeight.bold,
                                    fontSize: 8))),
                        pw.Padding(
                            padding: const pw.EdgeInsets.all(4),
                            child: pw.Text('12 Meses',
                                style: pw.TextStyle(
                                    fontWeight: pw.FontWeight.bold,
                                    fontSize: 8))),
                        pw.Padding(
                            padding: const pw.EdgeInsets.all(4),
                            child: pw.Text('Adulto (Est.)',
                                style: pw.TextStyle(
                                    fontWeight: pw.FontWeight.bold,
                                    fontSize: 8))),
                      ]),
                  pw.TableRow(children: [
                    ...
                    pw.Padding(
                        ...
                        child: pw.Text(
                            analysis.identificacao
                                    .curvaCrescimento['peso_12_meses']
                                    ?.toString() ??
                                '-',
                            style: const pw.TextStyle(fontSize: 8))),
                    pw.Padding(
          ...
          // Health Details
          pw.Text('2. ${strings.petSectionPreventive.toUpperCase()}',
              style:
                  pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 11)),
          pw.SizedBox(height: 5),
          if (analysis.saude.predisposicoesGeneticas.isNotEmpty)
            pw.Text('Genética: ${analysis.saude.predisposicoesGeneticas}',
                style: const pw.TextStyle(fontSize: 10)),
          if (analysis.saude.sinaisAlertaPrecoce.isNotEmpty)
            pw.Text('Sinais de Alerta: ${analysis.saude.sinaisAlertaPrecoce}',
                style:
                    const pw.TextStyle(fontSize: 10, color: PdfColors.red900)),
          pw.SizedBox(height: 10),

          // V472: Clinical Signs Logic (Eyes/Teeth/Skin)
          if (analysis.eyeDetails != null ||
              analysis.dentalDetails != null ||
              analysis.skinDetails != null) ...[
            pw.Text(strings.pdfClinicalSigns,
                style:
                    pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10)),
            pw.SizedBox(height: 4),
                ...
                children: [
                  if (analysis.eyeDetails != null)
                    pw.TableRow(children: [
                      ...
                      pw.Padding(
                          padding: const pw.EdgeInsets.all(4),
                          child: pw.Text(
                              analysis.eyeDetails!['characteristics']
                                      ?.toString() ??
                                  'N/A',
                              style: const pw.TextStyle(fontSize: 9))),
                    ]),
                  if (analysis.dentalDetails != null)
                    pw.TableRow(children: [
                      pw.Padding(
                          padding: const pw.EdgeInsets.all(4),
                          child: pw.Text(strings.pdfTeeth,
                              style: pw.TextStyle(
                                  fontWeight: pw.FontWeight.bold,
                                  fontSize: 9))),
                      pw.Padding(
                          padding: const pw.EdgeInsets.all(4),
                          child: pw.Text(
                              analysis.dentalDetails!['characteristics']
                                      ?.toString() ??
                                  'N/A',
                              style: const pw.TextStyle(fontSize: 9))),
                    ]),
                  if (analysis.skinDetails != null)
                    pw.TableRow(children: [
                      ...
                      pw.Padding(
                          padding: const pw.EdgeInsets.all(4),
                          child: pw.Text(
                              analysis.skinDetails!['characteristics']
                                      ?.toString() ??
                                  'N/A',
                              style: const pw.TextStyle(fontSize: 9))),
                    ]),
          ...
          // Nutrition
          pw.Text('3. ${strings.petSectionNutrition.toUpperCase()}',
              style:
                  pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 11)),
          pw.SizedBox(height: 5),
          pw.Bullet(
              text:
                  'Metas Calóricas: Adulto (${analysis.nutricao.metaCalorica['kcal_adulto']}), Filhote (${analysis.nutricao.metaCalorica['kcal_filhote']})'),
          pw.Bullet(
              text:
                  'Nutrientes Alvo: ${analysis.nutricao.nutrientesAlvo.join(", ")}'),

          pw.SizedBox(height: 15),

          // Lifestyle
          pw.Text('4. ${strings.petSectionLifestyle.toUpperCase()}',
              style:
                  pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 11)),
          pw.SizedBox(height: 5),
          pw.Bullet(
              text:
                  'Ambiente Ideal: ${analysis.lifestyle.ambienteIdeal['necessidade_de_espaco_aberto']}'),
          pw.Bullet(
              text:
                  'Nível de Atividade: ${analysis.lifestyle.estimuloMental['necessidade_estimulo_mental']}'),

          pw.SizedBox(height: 15),

          // 🛡️ V231: STOOL ANALYSIS DETAILS (ROBUST CHECK)
          if (analysis.analysisType == 'stool_analysis' ||
              (analysis.category?.toLowerCase().contains('fezes') ?? false) ||
          ...
          if (fullHistory.isNotEmpty) ...[
            pw.Text('5. ${strings.pdfAnaliseFeridas.toUpperCase()}',
                style:
                    pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 11)),
            pw.SizedBox(height: 5),
            ...fullHistory.asMap().entries.map((entry) {
              ...
              final h = entry.value;
              final dateStr =
                  DateFormat.yMd(strings.localeName).format(h.dataAnalise);
                      ...
                      children: [
                        ...
                        if (historyImg != null)
                          pw.Container(
                              ...
                              child:
                                  pw.Image(historyImg, fit: pw.BoxFit.cover)),
                        ...
                        pw.Expanded(
                            child: pw.Column(
                              ...
                              pw.Row(children: [
                                ...
                                pw.SizedBox(width: 8),
                                pw.Text((h.categoria ?? 'Geral').toUpperCase(),
                                    style: const pw.TextStyle(
                                        fontSize: 8, color: PdfColors.grey800)),
                              ]),
                              pw.SizedBox(height: 4),
                              ...
                              if (h.diagnosticosProvaveis.isNotEmpty)
                                pw.Text(
                                    'Diagnósticos: ${h.diagnosticosProvaveis.join(", ")}',
                                    style: pw.TextStyle(
                                        fontSize: 8,
                                        fontWeight: pw.FontWeight.bold,
                                        color: PdfColors.black)),
            ...
            pw.Text('5. ${strings.pdfAnaliseFeridas.toUpperCase()}',
                style:
                    pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 11)),
            pw.Text("Primeira Análise Registrada",
                style: pw.TextStyle(
                    fontSize: 9,
                    fontStyle: pw.FontStyle.italic,
                    color: PdfColors.grey600)),
    ...
    debugPrint(
        '✅ [V116-PDF] Relatório gerado com sucesso para o pet: $petName.');
    return pdf;
  }

  /// 🛡️ V231: Build Stool Analysis Section for PDF
  pw.Widget _buildStoolAnalysisSection(
      PetAnalysisResult analysis, AppLocalizations strings) {
    final details = analysis.stoolAnalysis ?? {};
    final PdfColor pdfRiskColor =
        analysis.urgenciaNivel.toLowerCase().contains('vermelho')
            ? PdfColors.red
            : (analysis.urgenciaNivel.toLowerCase().contains('amarelo')
                ? PdfColors.orange
                : PdfColors.green);
    ...
    return pw.Column(
      ...
      children: [
        pw.SizedBox(height: 15),
        pw.Text('5. ANÁLISE COPROLÓGICA (STOOL ANALYSIS)',
        ...
        pw.Container(
          ...
          child: pw.Column(
            ...
            children: [
                ...
                children: [
                  ...
                  pw.Container(
                    ...
                    decoration: pw.BoxDecoration(
                        ...
                        borderRadius:
                            const pw.BorderRadius.all(pw.Radius.circular(4))),
                    child: pw.Text(analysis.urgenciaNivel.toUpperCase(),
                        style: pw.TextStyle(
                            color: PdfColors.white,
                            fontWeight: pw.FontWeight.bold,
                            fontSize: 8)),
              ...
              pw.SizedBox(height: 10),
              _buildReportRow('Consistência (Bristol):',
                  'Escala ${details['consistency_bristol_scale'] ?? "-"}'),
              _buildReportRow(
                  'Textura/Firmeza:', details['firmness']?.toString() ?? '-'),
              _buildReportRow('Hidratação/Muco:',
                  details['hydration_mucus']?.toString() ?? '-'),
              _buildReportRow('Cor:',
                  '${details['color_name'] ?? "-"} (${details['color_hex'] ?? ""})'),
              _buildReportRow('Significado Clínico:',
                  details['clinical_color_meaning']?.toString() ?? '-'),
              _buildReportRow(
                  'Corpos Estranhos:',
              ...
              _buildReportRow(
                  'Parasitas Visíveis:',
                  (details['parasites_detected'] == true)
                      ? 'SIM'
                      : 'Não detectados'),
              _buildReportRow('Avaliação de Volume:',
                  details['volume_assessment']?.toString() ?? '-'),
              pw.Divider(thickness: 0.5, color: PdfColors.grey400, height: 16),
              pw.Text('DESCRIÇÃO VISUAL COMPLETA:',
                  style: pw.TextStyle(
                      fontWeight: pw.FontWeight.bold, fontSize: 8)),
              pw.SizedBox(height: 4),
              pw.Text(analysis.descricaoVisual,
        ...
        if (analysis.possiveisCausas.isNotEmpty) ...[
          pw.SizedBox(height: 10),
          pw.Text('DIAGNÓSTICOS DIFERENCIAIS:',
              style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 9)),
          ...analysis.possiveisCausas.map((c) =>
              pw.Bullet(text: c, style: const pw.TextStyle(fontSize: 8.5))),
        ],
        pw.SizedBox(height: 10),
        pw.Container(
          padding: const pw.EdgeInsets.all(8),
          decoration: pw.BoxDecoration(
              color: colorPetUltraLight,
              border: pw.Border.all(color: colorPet, width: 0.5)),
          child: pw.Column(
            crossAxisAlignment: pw.CrossAxisAlignment.start,
            children: [
              pw.Text('CONSELHO DO ESPECIALISTA:',
                  style: pw.TextStyle(
                      fontWeight: pw.FontWeight.bold,
                      fontSize: 9,
                      color: PdfColors.black)),
  ...
  pw.Widget _buildSafeBullet(String text,
      {double fontSize = 9, PdfColor? color}) {
    return pw.Padding(
        padding: const pw.EdgeInsets.only(bottom: 2),
        child:
            pw.Row(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
          ...
          pw.Expanded(
              child: pw.Text(text,
                  style:
                      pw.TextStyle(fontSize: fontSize, color: PdfColors.black)))
        ]));
  }
  ...
  Future<pw.Document> generateFoodAnalysisReport({
    ...
    pdf.addPage(
      pw.MultiPage(
          ...
          margin: const pw.EdgeInsets.all(35),
          header: (context) => buildHeader(
              '${strings.pdfFoodTitle} - 360º', timestampStr,
              color: colorFood),
          footer: (context) => buildFooter(context, strings: strings),
          build: (context) => [
                ...
                pw.Row(
                    ...
                    children: [
                      buildIndicator(
                          'Calorias (100g)', '$kcal kcal', colorFood),
                      buildIndicator(
                          strings.nutrientsCarbs, carbs, PdfColors.orange),
                      buildIndicator(
                          strings.nutrientsProteins, protein, PdfColors.red),
                      buildIndicator(
                          strings.nutrientsFats, fats, PdfColors.yellow),
                    ]),
                          ...
                          if (analysis.analise.pontosPositivos.isNotEmpty) ...[
                            pw.Text('PONTOS POSITIVOS:',
                                style: pw.TextStyle(
                                    ...
                                    fontSize: 10,
                                    color: PdfColors.green700)),
                            ...analysis.analise.pontosPositivos.map((e) =>
                                _buildSafeBullet(e, color: PdfColors.green700)),
                          ],
                          if (analysis.analise.pontosNegativos.isNotEmpty) ...[
                            ...
                            pw.Text('PONTOS DE ATENÇÃO:',
                                style: pw.TextStyle(
                                    ...
                                    fontSize: 10,
                                    color: PdfColors.red700)),
                            ...analysis.analise.pontosNegativos.map((e) =>
                                _buildSafeBullet(e, color: PdfColors.red700)),
                          ]
                        ])),
                ...
                buildSectionHeader(
                    (strings.foodBiohacking ?? 'BIOHACKING').toUpperCase(),
                    color: colorFood),
                pw.Row(
                    crossAxisAlignment: pw.CrossAxisAlignment.start,
                    children: [
                          ...
                          child: pw.Column(
                            ...
                            pw.Text('BENEFÍCIOS AO CORPO:',
                                style: pw.TextStyle(
                                    fontWeight: pw.FontWeight.bold,
                                    fontSize: 10)),
                            ...analysis.performance.pontosPositivosCorpo.map(
                                (e) => _buildSafeBullet(e, color: colorFood)),
                            pw.SizedBox(height: 10),
                            ...
                            if (analysis.performance.pontosAtencaoCorpo.isEmpty)
                              ...
                              ...analysis.performance.pontosAtencaoCorpo.map(
                                  (e) => _buildSafeBullet(e,
                                      color: PdfColors.orange)),
                          ])),
                      ...
                      pw.Expanded(
                          child: pw.Column(
                              ...
                              children: [
                            buildIndicator(
                                strings.foodSatietyIndex ?? 'Saciedade',
                                '${analysis.performance.indiceSaciedade}/10',
                                PdfColors.teal),
                            pw.SizedBox(height: 10),
                            pw.Text('FOCO & ENERGIA:',
                                style: pw.TextStyle(
                                    fontWeight: pw.FontWeight.bold,
                                    fontSize: 10)),
                            pw.Text(analysis.performance.impactoFocoEnergia,
                                style: const pw.TextStyle(fontSize: 9)),
                            pw.SizedBox(height: 10),
                            pw.Text('MOMENTO IDEAL:',
                                style: pw.TextStyle(
                                    fontWeight: pw.FontWeight.bold,
                                    fontSize: 10)),
                            pw.Text(analysis.performance.momentoIdealConsumo,
                                style: const pw.TextStyle(fontSize: 9)),
                          ])),
                    ]),
                    ...
                    children: [
                      pw.Expanded(
                          child: pw.Column(
                              ...
                              children: [
                            buildSectionHeader(
                                strings.foodSafetyBio ?? 'SEGURANÇA',
                                color: colorFood),
                            if (analysis.identidade.alertaCritico.isNotEmpty &&
                                analysis.identidade.alertaCritico != 'Nenhum')
                              pw.Container(
                                padding: const pw.EdgeInsets.all(8),
                                decoration: pw.BoxDecoration(
                                    color: PdfColors.red50,
                                    border:
                                        pw.Border.all(color: PdfColors.red)),
                                child: pw.Text(
                                    '⚠️ ${analysis.identidade.alertaCritico}',
                                    style: pw.TextStyle(
                                        color: PdfColors.red900,
                                        fontSize: 9,
                                        fontWeight: pw.FontWeight.bold)),
                            ...
                            pw.Text('BIOQUÍMICA:',
                                style: pw.TextStyle(
                                    fontWeight: pw.FontWeight.bold,
                                    fontSize: 10)),
                            pw.Text(analysis.identidade.bioquimicaAlert,
                                style: const pw.TextStyle(fontSize: 9)),
                          ])),
                      pw.SizedBox(width: 15),
                      pw.Expanded(
                          child: pw.Column(
                            ...
                            pw.Text('SINERGIA NUTRICIONAL:',
                                style: pw.TextStyle(
                                    ...
                                    fontSize: 10)),
                            pw.Text(
                                analysis.micronutrientes.sinergiaNutricional,
                                style: const pw.TextStyle(fontSize: 9)),
                            pw.SizedBox(height: 5),
                            pw.Wrap(
                                spacing: 4,
                                runSpacing: 4,
                                children:
                                    analysis.micronutrientes.lista.map((m) {
                                  return pw.Container(
                                      padding: const pw.EdgeInsets.symmetric(
                                          horizontal: 4, vertical: 2),
                                      decoration: const pw.BoxDecoration(
                ...
                buildSectionHeader(
                    strings.recipesCulinaryIntel ?? 'GASTRONOMIA',
                    color: colorFood),
                pw.Row(children: [
                  pw.Expanded(
                ...
                if (analysis.receitas.isNotEmpty) ...[
                  pw.SizedBox(height: 10),
                  pw.Text(
                      (strings.recipesQuick ?? 'RECEITAS SUGERIDAS')
                          .toUpperCase(),
                      style: pw.TextStyle(
  ...
  String _getBotanyHistoryToxicityText(BotanyHistoryItem item) {
    String warning =
        'Mantenha fora do alcance de crianças e animais de estimação. Em caso de ingestão, procure atendimento médico imediatamente.';
    final meta = item.rawMetadata;
    if (meta != null && meta['seguranca_biofilia'] != null) {
      final sec = meta['seguranca_biofilia']['seguranca_domestica'];
      if (sec != null) {
        ...
        if (details.contains('gato') && details.contains('cão')) {
          warning =
              'ALERTA: Esta planta é TÓXICA para CÃES e GATOS. Mantenha em locais inacessíveis (prateleiras altas ou suspensas).';
        } else if (details.contains('gato') || details.contains('felino')) {
          warning =
              'ALERTA: Esta planta é TÓXICA para GATOS. Evite tê-la em casa se possuir felinos, pois o contato pode causar insuficiência renal grave.';
        } else if (details.contains('cão') || details.contains('cachorro')) {
          warning =
              'ALERTA: Esta planta é TÓXICA para CÃES. Mantenha fora do alcance do seu cachorro.';
        }
      }
    }
    return warning;
