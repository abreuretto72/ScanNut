...
class _PartnerEventDetailScreenState extends State<PartnerEventDetailScreen> {
  ...
  Future<void> _openEditModal() async {
    ...
    if (eventMap == null) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content:
                Text(AppLocalizations.of(context)!.agendaOriginalDataMissing)));
      }
      return;
    }
        ...
        builder: (context, scrollController) => Container(
          ...
          child: AddEventModal(
            ...
            onSave: (updatedEvent) async {
              ...
              final updatedEvents =
                  List<Map<String, dynamic>>.from(agendaEvents);
              final index =
                  updatedEvents.indexWhere((e) => e['id'] == agendaEvent.id);
              if (index != -1) {
              ...
              final pType = updatedEvent.category.name == 'vacina'
                  ? EventType.vaccine
                  : updatedEvent.category.name == 'banho'
                      ? EventType.bath
                      : updatedEvent.category.name == 'tosa'
                          ? EventType.grooming
                          : updatedEvent.category.name == 'consulta'
                              ? EventType.veterinary
                              : updatedEvent.category.name == 'remedios'
              ...
              if (mounted) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content:
                        Text(AppLocalizations.of(context)!.agendaEventUpdated),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: AppDesign.backgroundDark,
        title: Text(AppLocalizations.of(context)!.agendaServiceRecord,
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (_partner != null)
                    ...
                    Text(AppLocalizations.of(context)!.agendaPartnerNotFound,
                  ...
                  if (!_event.completed) ...[
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        ...
                        label: Text(
                            AppLocalizations.of(context)!.agendaMarkCompleted),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildDetailRow(
              Icons.access_time,
              AppLocalizations.of(context)!.pdfFieldTime,
          ...
          if (attendantName != null) ...[
            _buildDetailRow(
                Icons.person,
                AppLocalizations.of(context)!.partnerDetailsRole,
                attendantName),
            const SizedBox(height: 12),
          ],
          ...
          InkWell(
            onTap: () async {
              ...
              if (profileData != null && mounted) {
                debugPrint(
                    'Navegando para o Pet: ${_event.petName} | Foto: ${profileData['photo_path']} | Vínculos: ${profileData['data']?['linked_partner_ids']}');
                ...
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Text('Perfil não encontrado: ${_event.petName}')));
              }
            },
            child: Row(
              children: [
                ...
                const Text('Pet: ',
                    style: TextStyle(color: AppDesign.textSecondaryDark)),
                Expanded(
                  child: Text(
          ...
          if (displayNotes.isNotEmpty) ...[
            const Divider(color: Colors.white12, height: 32),
            Text('${AppLocalizations.of(context)!.pdfObservations}:',
                style: const TextStyle(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          Text(
            _partner!.specialties.isNotEmpty
                ? _partner!.specialties.join(', ')
                : _partner!.category,
          ...
          InkWell(
            onTap: () async {
              await Navigator.push(
                  ...
                  MaterialPageRoute(
                      builder: (_) =>
                          PartnerRegistrationScreen(initialData: _partner!)));
          ...
          if (_partner!.address.isNotEmpty)
            _buildActionRow(Icons.location_on, _partner!.address,
                () => _openMap(_partner!.address)),
          const SizedBox(height: 12),
          if (_partner!.phone.isNotEmpty)
            _buildActionRow(Icons.phone, _partner!.phone,
                () => _makePhoneCall(_partner!.phone)),
          const SizedBox(height: 12),
          if (_partner!.whatsapp != null && _partner!.whatsapp!.isNotEmpty)
            _buildActionRow(
                Icons.chat,
                AppLocalizations.of(context)!.agendaWhatsAppChat,
                () => _openWhatsApp(_partner!.whatsapp!))
          else if (_partner!.phone.isNotEmpty)
            _buildActionRow(
                Icons.chat,
                AppLocalizations.of(context)!.agendaWhatsAppChat,
                () => _openWhatsApp(_partner!.phone)),
          const SizedBox(height: 12),
          ...
          if (_partner!.website != null && _partner!.website!.isNotEmpty)
            _buildActionRow(Icons.language, _partner!.website!, () async {
              ...
              if (!await launchUrl(url)) {
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Text(
                        AppLocalizations.of(context)!.agendaWebsiteError)));
              }
            }),
        ],
      ),
          ...
          children: [
            ...
            if (onTap != null)
              const Icon(Icons.arrow_forward_ios,
                  color: AppDesign.textSecondaryDark, size: 12),
          ],
        ),
      ),
    );
