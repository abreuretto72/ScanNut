...
class AnalysisResultsFragment extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final hasData =
        history.isNotEmpty || (current != null && current.isNotEmpty);
      ...
      child: Column(
        children: [
          ...
          if (!hasData)
            ...
            ...List.generate(
                history.length +
                    (current != null && current.isNotEmpty && history.isEmpty
                        ? 1
                        : 0) +
                    1, (index) {
              if (index == 0) {
              ...
              if (data.containsKey('identification') &&
                  (rawType.isEmpty || rawType == 'identification')) {
              ...
              String? extractedBreed =
                  data['breed']?.toString() ?? findBreedRecursive(data);

              if (extractedBreed == null) {
                final str = data.toString();
                final match = RegExp(r'(?:breed|raca)[:]\s*([^,}\]]+)',
                        caseSensitive: false)
                    .firstMatch(str);
                if (match != null) extractedBreed = match.group(1)?.trim();
              }

              String petNameFn = data['pet_name']?.toString() ??
                  data['name']?.toString() ??
                  '';
                ...
                child: Column(
                    ...
                    children: [
                      ...
                      ...data.entries.where((e) {
                        ...
                        if (v == null || v.toString().toLowerCase() == 'null') {
                          return false;
                        }
                        ...
                        if (val is Map) {
                          return ExpansionTile(
                            ...
                            title: Text(tryLocalizeLabel(context, e.key),
                                style: const TextStyle(
                                    color: Colors.white,
                                    fontSize: 12,
                                    fontWeight: FontWeight.bold)),
                            ...
                            children: (val)
                                ...
                                .map((sub) => Padding(
                                      ...
                                      child: Row(
                                          ...
                                          children: [
                                            Text(
                                                '${tryLocalizeLabel(context, sub.key)}: ',
                                                style: const TextStyle(
                        ...
                        return Padding(
                          ...
                          child: Row(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text('${tryLocalizeLabel(context, e.key)}: ',
                                    style: const TextStyle(
                                        color: AppDesign.petPink,
                                        fontSize: 11,
                                        fontWeight: FontWeight.bold)),
    ...
    return InkWell(
        ...
        child: Container(
          ...
          child: Row(children: [
            ...
            Expanded(
                child: Text(l10n.petAnalysisViewImage,
                    style: const TextStyle(
                        color: AppDesign.petPink,
                        fontWeight: FontWeight.bold,
                        fontSize: 12))),
