...
class TravelFragment extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      padding: const EdgeInsets.fromLTRB(
          24, 24, 24, 100), // Padding lower 100.0 to avoid footer
      child: Column(
        ...
        children: [
                ...
                children: [
                  ...
                  Expanded(
                    child: Text(
                      ...
                      style: TextStyle(
                          color: Colors.amber[100], fontSize: 12, height: 1.4),
                    ),
                  ),
                ],
              ),
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  if (especie?.toLowerCase().contains('cão') == true ||
                      especie?.toLowerCase().contains('cao') == true ||
                      especie?.toLowerCase().contains('dog') == true) ...[
                    const Divider(color: Colors.white10, height: 32),
                    AttachmentSection(
                  ...
                  ] else if (especie?.toLowerCase().contains('gato') == true ||
                      especie?.toLowerCase().contains('cat') == true) ...[
                    const Divider(color: Colors.white10, height: 32),
                    AttachmentSection(
          ...
          _buildTravelSection(context, l10n.travel_section_car,
              Icons.directions_car, l10n.travel_car_tips, [
          ...
          _buildTravelSection(context, l10n.travel_section_plane,
              Icons.airplanemode_active, l10n.travel_plane_checklist, [
          ...
          _buildTravelSection(context, l10n.travel_section_ship,
              Icons.directions_boat, l10n.travel_ship_tips, [
  ...
  Widget _buildValidationCard(BuildContext context, String title, bool isValid,
  ...
  Widget _buildTravelSection(BuildContext context, String title, IconData icon,
      String tips, List<String> checklist) {
    return Card(
      color: Colors.white.withValues(alpha: 0.05),
  ...
  Widget _buildInteractiveCheck(BuildContext context, String label, bool value,
  ...
  List<Widget> _buildIntelligentChecklist(
    ...
    if (analysisHistory.isEmpty &&
        labExams.isEmpty &&
        historicoAnaliseFeridas.isEmpty) {
      items.add(_buildStaticCheck(context, l10n.petTravelHealthCheckup, false,
          isWarning: true));
    } else {
      // 1. Parasites
      ...
      if (hasParasites) {
        items.add(_buildStaticCheck(context, l10n.petTravelHygieneKit, true,
      ...
      if (hasInfection) {
        items.add(_buildStaticCheck(
            context, l10n.petTravelHydrationMonitoring, true,
            isWarning: true, subtitle: 'Baseado em exames laboratoriais'));
      }
      ...
      for (var a in analysisHistory) {
        final score = int.tryParse(
                a['reliability']?.toString().replaceAll('%', '') ?? '100') ??
            100; // Simplified score check
      ...
      if (needsRest) {
        items.add(_buildStaticCheck(context, l10n.petTravelRestSupport, true,
            isWarning: true));
      }
      ...
      if (needsPremiumFood) {
        items.add(
            _buildStaticCheck(context, l10n.petTravelPremiumFoodKit, true));
      }
    }

    // 5. Standard Educational Items
    items.add(_buildStaticCheck(context, l10n.petTravelMedicationActive, false,
        subtitle: l10n.petTravelMedicationActiveDesc));
    items.add(_buildStaticCheck(context, l10n.petTravelWaterMineral, false,
        subtitle: l10n.petTravelWaterMineralDesc));
    items.add(_buildStaticCheck(context, l10n.petTravelTacticalStops, false,
        subtitle: l10n.petTravelTacticalStopsDesc));

    // 6. Basic Checklist Items (Interactive)
    items.add(_buildInteractiveCheck(
        context,
        l10n.petTravelSafetyBelt,
        prefsMap['has_safety_belt'] == true,
        (val) => onPreferenceChanged('has_safety_belt', val)));
    items.add(_buildInteractiveCheck(
        context,
        l10n.petTravelHealthCert,
        prefsMap['has_health_cert'] == true,
        (val) => onPreferenceChanged('has_health_cert', val)));
    items.add(_buildInteractiveCheck(
        context,
        l10n.petTravelCZI,
        prefsMap['has_czi'] == true,
        (val) => onPreferenceChanged('has_czi', val)));

    return items;
  }

  List<Widget> _buildVaccinationGuide(
      BuildContext context, AppLocalizations l10n) {
    final List<Widget> items = [];
    final bool isDog = especie?.toLowerCase().contains('cão') == true ||
        especie?.toLowerCase().contains('cao') == true ||
        especie?.toLowerCase().contains('dog') == true;
    final bool isCat = especie?.toLowerCase().contains('gato') == true ||
        especie?.toLowerCase().contains('cat') == true;

    if (isDog) {
      items.add(_buildEducationalInfo(
          context, 'V8/V10 (Cães)', l10n.petTravelV8V10Desc, Icons.shield));
      items.add(_buildEducationalInfo(
          context, 'Gripe/Bordetella', l10n.petTravelGripeDesc, Icons.air));
      items.add(_buildEducationalInfo(
          context, 'Leishmaniose', l10n.petTravelLeishDesc, Icons.bug_report));
    } else if (isCat) {
      items.add(_buildEducationalInfo(
          context, 'V3/V4/V5 (Gatos)', l10n.petTravelV3V4V5Desc, Icons.shield));
    }

    // Both
    items.add(_buildEducationalInfo(
        context, 'Antirrábica', l10n.petTravelRabiesDesc, Icons.gavel,
        isMandatory: true));

    return items;
  }

  Widget _buildStaticCheck(BuildContext context, String label, bool isAutomated,
      {bool isWarning = false, String? subtitle}) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
            color:
                isWarning ? Colors.red.withValues(alpha: 0.3) : Colors.white10),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(icon,
                  color: isMandatory ? AppDesign.petPink : Colors.white38,
                  size: 20),
              const SizedBox(width: 12),
              Expanded(
                child: Text(
