...
class _PetBodyAnalysisCardState extends State<PetBodyAnalysisCard> {
  ...
  Future<void> _autoSave(Map<String, dynamic> data, String tempPath) async {
    ...
    try {
      ...
      try {
        ...
        final simpleAdvice =
            data['simple_advice']?.toString() ?? 'Sem recomendações.';

        // Validação ID
        final effectivePetId = petId.isNotEmpty ? petId : widget.petName;
        debugPrint(
            '🔍 [TRACE-BODY] Target ID: $effectivePetId, Name: ${widget.petName}');
  ...
  Widget build(BuildContext context) {
    final strings = AppLocalizations.of(context);
    final scoreColor = _lastResult != null
        ? _getScoreColor(_lastResult?['health_score'])
        : Colors.grey;
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                strings?.petBodyAnalysisDesc ??
                    'Avalie bem-estar físico e sinais de dor.',
                style: TextStyle(
                ...
                Center(
                  child: Column(
                    children: [
                      ...
                      Text(strings?.petBodyProcessing ?? 'Analisando...',
                          style: const TextStyle(
              ...
              if (_lastResult != null) ...[
                ...
                _buildResultRow(
                    Icons.favorite,
                    strings?.petBodyHealthScore ?? 'Nível Saúde',
                    '${_lastResult?['health_score']}/10',
                    color: scoreColor),
                _buildResultRow(
                    Icons.visibility,
                    strings?.petBodySignals ?? 'Sinais',
                    _lastResult?['body_signals']?.toString()),
                _buildResultRow(
                    Icons.health_and_safety,
                    strings?.petBodyAdvice ?? 'Dica',
                    _lastResult?['simple_advice']?.toString()),
                const SizedBox(height: 12),
                Align(
              ...
              if (widget.analysisHistory
                  .any((a) => a['analysis_type'] == 'body_analysis')) ...[
                const SizedBox(height: 24),
                const Divider(color: Colors.white10),
                const SizedBox(height: 8),
                const Text('Histórico Postural',
                    style: TextStyle(
