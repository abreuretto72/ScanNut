...
class _PetFoodAnalysisCardState extends State<PetFoodAnalysisCard> {
  ...
  Future<void> _analyze(String path) async {
    ...
    try {
      final result = await GeminiService().analyzePetFood(path);
  ...
  Future<void> _autoSave(Map<String, dynamic> data, String tempPath) async {
    try {
       ...
       final analysisForHistory = {
          ...
          'original_filename': p.basename(tempPath), // 📸 NOME DA IMAGEM
          'veredit': data['veredit'],
          'simple_reason': data['simple_reason'],
          'daily_tip': data['daily_tip'],
          'image_path': finalPath, // 🛡️ GUARDANDO A IMAGEM
          'last_updated': DateTime.now().toIso8601String(),
       };

       await PetProfileService().addAnalysisToHistory(widget.petName, analysisForHistory);

       // 3. Save to PetEvents (for timeline)
       final service = PetEventService();
       await service.init();
       if (!service.box.isOpen) await service.init();
       
       final veredit = data['veredit'] ?? 'N/A';
       final reason = data['simple_reason'] ?? '';
       ...
       final event = PetEvent(
          ...
          title: 'Análise de Ração: $veredit',
          type: EventType.food, 
       ...
       debugPrint('✅ [PetFood] Auto-saved to Hive (History & Events).');
    } catch (e) {
       debugPrint('❌ [PetFood] Save failed: $e');
    }
  }
  
  Color _getVereditColor(String? veredit) {
     if (veredit == null) return Colors.grey;
     final v = veredit.toLowerCase();
     if (v.contains('boa')) return Colors.green;
     if (v.contains('regular')) return Colors.amber;
     if (v.contains('ruim') || v.contains('pessima')) return Colors.red;
     return Colors.blue;
  }
  ...
  Widget build(BuildContext context) {
    final strings = AppLocalizations.of(context);
    final vereditColor = _lastResult != null ? _getVereditColor(_lastResult?['veredit']?.toString()) : Colors.grey;
            ...
            children: [
               ...
               if (_isProcessing)
                ...
                if (_lastResult != null) ...[
                   ...
                   _buildResultRow(Icons.grade, strings?.petFoodVerdict ?? 'Veredito', _lastResult?['veredit']?.toString(), color: vereditColor),
                   _buildResultRow(Icons.info_outline, strings?.petFoodReason ?? 'Motivo', _lastResult?['simple_reason']?.toString()),
                   _buildResultRow(Icons.lightbulb, strings?.petFoodTip ?? 'Dica', _lastResult?['daily_tip']?.toString()),
  ...
  Widget _buildFoodHistoryItem(Map<String, dynamic> item) {
     ...
     final color = _getVereditColor(veredit);
