...
class _PetFoodAnalysisCardState extends State<PetFoodAnalysisCard> {
    ...
    try {
      ...
      if (hash.isNotEmpty) {
        ...
        if (existing != null) {
          ...
          if (mounted) {
            setState(() {
              _errorMessage = strings?.error_image_already_analyzed ??
                  'Imagem já analisada.';
              _isProcessing = false;
            });
          }
      ...
      // 1. Fetch Pet Context
      final profileMap =
          await PetProfileService().getProfile(widget.petId ?? widget.petName);
      String? age;
      String? breedSpecies;
      String? weight;
  ...
  Future<void> _autoSave(Map<String, dynamic> data, String tempPath) async {
    try {
      ...
      if (await file.exists()) {
        ...
        finalPath = await vault.secureClone(
            file, MediaVaultService.PETS_DIR, widget.petName, true);
      }
      ...
      await PetProfileService().addAnalysisToHistory(
          widget.petId ?? widget.petName, analysisForHistory);
      ...
      if (sugestoes.isNotEmpty) {
        final sName =
            sugestoes.first['marka'] ?? sugestoes.first['marca'] ?? '';
  ...
  Color _getVereditColor(Map<String, dynamic>? result) {
    ...
    if (result['data'] != null && result['data'] is Map) {
      final innerFeedback =
          result['data']['feedback_visual']?.toString().toLowerCase();
      if (innerFeedback == 'saudavel') return Colors.green;
      if (innerFeedback == 'alerta' || innerFeedback == 'critico') {
        return Colors.red;
      }
    }
    ...
    if (veredit.contains('ruim') || veredit.contains('pessima')) {
      return Colors.red;
    }
  ...
  Widget build(BuildContext context) {
    final strings = AppLocalizations.of(context);
    final vereditColor =
        _lastResult != null ? _getVereditColor(_lastResult) : Colors.grey;
    final bgColor = _errorMessage != null
        ? Colors.red.withValues(alpha: 0.1)
        : (_lastResult != null
            ? vereditColor.withValues(alpha: 0.1)
            ...
            children: [
              ...
              if (!_isProcessing && _lastResult == null)
                Row(
                  ...
                  children: [
                    _buildInputBtn(Icons.camera_alt, 'Câmera',
                        () => _pickImage(ImageSource.camera)),
                    _buildInputBtn(Icons.image, 'Galeria',
              ...
              if (_lastResult != null) ...[
                ...
                if (_lastResult?['image_path'] != null &&
                    File(_lastResult!['image_path']).existsSync())
                  Padding(
              ...
              if (widget.analysisHistory
                  .any((a) => a['analysis_type'] == 'food_label')) ...[
                const SizedBox(height: 24),
                const Divider(color: Colors.white10),
                const SizedBox(height: 8),
                const Text('Rótulos Analisados',
                    style: TextStyle(
  ...
  Widget _buildNutritionalDetails(Map<String, dynamic> result) {
    final data =
        result.containsKey('analise_rotulo') ? result : (result['data'] ?? {});
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildResultRow(Icons.branding_watermark, 'Marca Identificada',
            analysis['marca'] ?? 'Desconhecida'),
        _buildResultRow(
            Icons.verified, 'Qualidade', analysis['qualidade'] ?? 'Standard',
            color: _getVereditColor(result)),
        const SizedBox(height: 8),
        const Text('Níveis Garantidos:',
            style: TextStyle(
                color: Colors.white70,
                fontSize: 11,
                fontWeight: FontWeight.bold)),
          ...
          children: [
            Expanded(
                child: _buildNutrientChip(
                    'Proteína', nutrients['proteina'] ?? '0%')),
            const SizedBox(width: 8),
            Expanded(
                child: _buildNutrientChip(
                    'Gordura', nutrients['gordura'] ?? '0%')),
            const SizedBox(width: 8),
            Expanded(
                child:
  ...
  Widget _buildSuggestions(Map<String, dynamic> result) {
    final data =
        result.containsKey('sugestoes') ? result : (result['data'] ?? {});
    final sugestoes = data['sugestoes'] as List<dynamic>? ?? [];
      ...
      children: [
        ...
        ...sugestoes.map((s) => Container(
              ...
              decoration: BoxDecoration(
                ...
                border:
                    Border.all(color: AppDesign.petPink.withValues(alpha: 0.2)),
              ),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(s['marca'] ?? 'Marca Sugerida',
                      style: const TextStyle(
                          color: Colors.white,
                          fontWeight: FontWeight.bold,
                          fontSize: 14)),
  ...
  Widget _buildDisclaimer(Map<String, dynamic> result) {
    final data =
        result.containsKey('aviso_legal') ? result : (result['data'] ?? {});
    final aviso =
        data['aviso_legal'] ?? 'Consulta ao Veterinário é indispensável.';
