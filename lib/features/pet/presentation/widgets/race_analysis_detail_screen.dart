...
class RaceAnalysisDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // 1. Identification
    Map? rawIdent = (raceAnalysis['identification'] ??
        raceAnalysis['identificacao']) as Map?;
    ...
    if (rawIdent != null) {
      ident = {
        ...
        'racas_secundarias':
            rawIdent['secondary_breeds'] ?? rawIdent['racas_secundarias'],
    ...
    // 2. Temperament
    Map? rawTemp =
        (raceAnalysis['temperament'] ?? raceAnalysis['temperamento']) as Map?;
    Map<String, dynamic>? temp;
    if (rawTemp != null) {
      temp = {
        'personalidade': rawTemp['personality'] ?? rawTemp['personalidade'],
        'comportamento_social':
            rawTemp['social_behavior'] ?? rawTemp['comportamento_social'],
        'nivel_energia': rawTemp['energy_level'] ?? rawTemp['nivel_energia'],
      };
    }

    // 3. Physical
    Map? rawFisica = (raceAnalysis['characteristics'] ??
        raceAnalysis['caracteristicas_fisicas']) as Map?;
    ...
    if (rawFisica != null) {
      fisica = {
        ...
        'expectativa_vida': rawFisica['lifespan'] ??
            rawFisica['longevity'] ??
            rawFisica['expectativa_vida'],
    ...
    // 4. Care
    Map? rawCuidados =
        (raceAnalysis['care'] ?? raceAnalysis['cuidados']) as Map?;
    ...
    final origem = (raceAnalysis['origin'] ??
            raceAnalysis['origin_history'] ??
            raceAnalysis['origem'] ??
            raceAnalysis['origem_historia'])
        ?.toString();
    final curiosidades = (raceAnalysis['curiosities'] ??
        raceAnalysis['fun_facts'] ??
        raceAnalysis['curiosidades']) as List?;
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            if (ident != null) ...[
              _buildSectionTitle(
                  AppLocalizations.of(context)!.petGeneticId, Icons.science),
              const SizedBox(height: 12),
              _buildInfoCard([
                _buildInfoRow(
                    AppLocalizations.of(context)!.petLineage,
                    ident['linhagem_mista']?.toString() ??
                        AppLocalizations.of(context)!.petNotIdentified),
                _buildInfoRow(
                    AppLocalizations.of(context)!.petPrimaryRace,
                    ident['raca_predominante']?.toString() ??
                        AppLocalizations.of(context)!.petNotIdentified),
                _buildInfoRow(
                    AppLocalizations.of(context)!.petSecondaryRaces,
                    ident['racas_secundarias']?.toString() ??
                        AppLocalizations.of(context)!.petNotIdentifiedPlural),
                _buildInfoRow(
                    AppLocalizations.of(context)!.petReliability,
                    ident['confiabilidade']?.toString() ??
                        AppLocalizations.of(context)!.petReliabilityLow),
              ]),
            ...
            if (fisica != null) ...[
              _buildSectionTitle(AppLocalizations.of(context)!.petPhysicalChars,
                  Icons.straighten),
              const SizedBox(height: 12),
              _buildInfoCard([
                _buildInfoRow(
                    AppLocalizations.of(context)!.petSize,
                    fisica['porte']?.toString() ??
                        AppLocalizations.of(context)!.petNotIdentified),
                _buildInfoRow(
                    AppLocalizations.of(context)!.petWeightEstimated,
                    fisica['peso_estimado']?.toString() ??
                        AppLocalizations.of(context)!.petVariable),
                _buildInfoRow(
                    AppLocalizations.of(context)!.petHeight,
                    fisica['altura']?.toString() ??
                        AppLocalizations.of(context)!.petNotEstimated),
                _buildInfoRow(
                    AppLocalizations.of(context)!.petExpectancy,
                    fisica['expectativa_vida']?.toString() ??
                        AppLocalizations.of(context)!.petNotEstimated),
                _buildInfoRow(
                    AppLocalizations.of(context)!.petCoatType,
                    fisica['tipo_pelagem']?.toString() ??
                        AppLocalizations.of(context)!.petNotIdentified),
                _buildInfoRow(
                    AppLocalizations.of(context)!.petCommonColors,
                    fisica['cores_comuns']?.toString() ??
                        AppLocalizations.of(context)!.petVaried),
              ]),
              const SizedBox(height: 20),
            ],
            ...
            if (temp != null) ...[
              _buildSectionTitle(
                  AppLocalizations.of(context)!.petTemperamentTitle,
                  Icons.psychology),
              const SizedBox(height: 12),
              _buildInfoCard(temp.entries
                  .map((e) =>
                      _buildInfoRow(e.key.toUpperCase(), e.value.toString()))
                  .toList()),
              const SizedBox(height: 20),
            ],
            ...
            if (cuidados != null) ...[
              _buildSectionTitle(
                  AppLocalizations.of(context)!.petRecommendedCare,
                  Icons.favorite),
              const SizedBox(height: 12),
              _buildInfoCard(cuidados.entries
                  .map((e) =>
                      _buildCareItem(e.key.toUpperCase(), e.value.toString()))
                  .toList()),
              const SizedBox(height: 20),
            ],
            ...
            if (origem != null && origem.isNotEmpty) ...[
              _buildSectionTitle(AppLocalizations.of(context)!.petOriginHistory,
                  Icons.history_edu),
              const SizedBox(height: 12),
              Container(
                padding: const EdgeInsets.all(16),
            ...
            if (curiosidades != null && curiosidades.isNotEmpty) ...[
              _buildSectionTitle(
                  AppLocalizations.of(context)!.petCuriositiesTitle,
                  Icons.lightbulb),
              const SizedBox(height: 12),
              ...curiosidades.map((curiosidade) => Padding(
                  ...
                  child: Row(children: [
                    ...
                    Expanded(
                        child: Text(curiosidade.toString(),
                            style: GoogleFonts.poppins(
