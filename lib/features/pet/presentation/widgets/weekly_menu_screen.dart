...
class _WeeklyMenuScreenState extends ConsumerState<WeeklyMenuScreen>
  ...
  Future<void> _loadHistory() async {
    ...
    try {
      ...
      final plans = await MealPlanService().getPlansForPet(widget.petId.trim(),
          fallbackName: widget.petName.trim());
      if (mounted) {
        setState(() {
          _history = plans;
          _isLoading = false;
  ...
  Future<void> _deletePlan(String planId) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text(l10n.petTabHistory,
            style: const TextStyle(color: Colors.white)),
        content: Text(l10n.petMenuDeleteWeekConfirm,
          ...
          TextButton(
              ...
              child: Text(l10n.commonDelete,
                  style: const TextStyle(
                      color: colorDeepPink, fontWeight: FontWeight.bold))),
        ],
      ),
    );
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(l10n.petMenuDeletedSuccess),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text('Error: $e'), backgroundColor: AppDesign.error));
  ...
  Future<void> _generatePdfMulti() async {
    ...
    try {
      // 1. Gather Data
      final selectedPlans =
          _history.where((p) => _selectedPlanIds.contains(p.id)).toList();
      selectedPlans.sort(
          (a, b) => a.startDate.compareTo(b.startDate)); // Chronological order
      ...
      for (var plan in selectedPlans) {
        ...
        for (var day in plan.meals) {
          consolidatedPlan.add({
            ...
            'benefit': day.benefit ?? '',
            'refeicoes': [
              // Format expected by ExportService
              {
                'hora': day.time,
                'titulo': day.title,
                'descricao': day.description,
      ...
      await ExportService().generateWeeklyMenuReport(
        ...
        period: _selectedPlanIds.length > 1
            ? l10n.petMenuGeneratePdfMulti(selectedPlans.length)
            : null,
      ...
      if (mounted) {
        ...
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text('Pdf Error: $e'), backgroundColor: AppDesign.error));
      }
    }
  }
  ...
  String _formatDateForPdf(DateTime start, int dayOfWeekIso) {
    ...
    return DateFormat('EEEE dd/MM', Localizations.localeOf(context).toString())
        .format(d);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: const Color(0xFF1E1E1E),
      appBar: AppBar(
        title: Text(widget.petName.isEmpty ? 'Cardápio' : widget.petName,
          ...
          children: [
            Container(
              ...
              child: const Icon(Icons.auto_awesome,
                  size: 60, color: AppDesign.petPink),
            ),
              ...
              child: ElevatedButton(
                ...
                onPressed: () async {
                  ...
                  if (profile != null && profile['data'] != null) {
                    ...
                    bool valid =
                        (species != null && species.toString().isNotEmpty) &&
                    ...
                    if (!valid) {
                      ...
                      showDialog(
                          context: context,
                          builder: (ctx) => AlertDialog(
                                title: Text(l10n.petProfileIncompleteTitle,
                                    style: const TextStyle(
                                        color: AppDesign.textPrimaryDark)),
                                content: Text(l10n.petProfileIncompleteBody,
                                    style:
  ...
  Future<void> _runGeneration(
      Map<String, dynamic> config, Map<String, dynamic> fullProfileData) async {
    final l10n = AppLocalizations.of(context)!;
          ...
          child: Column(
            ...
            children: [
              const LinearProgressIndicator(
                  color: AppDesign.petPink, backgroundColor: Colors.white10),
    ...
    try {
      ...
      Map<String, dynamic> profileData =
          Map<String, dynamic>.from(fullProfileData);
      ...
      final request = MealPlanRequest(
        ...
        source:
            'PetProfile', // Authorized Source (WeeklyMenuScreen is reached via PetProfile)
      );
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(l10n.petMenuSuccess),
            backgroundColor: AppDesign.success));
        ...
        if (_selectedPlanIds.isNotEmpty)
          Container(
            ...
            child: Row(
              ...
              children: [
                Text(l10n.petMenuGeneratePdfMulti(_selectedPlanIds.length),
                    style: const TextStyle(
                ...
                Row(
                  children: [
                    TextButton.icon(
                      ...
                      label: Text(l10n.petMenuSelectionClear,
                          style: const TextStyle(
  ...
  Widget _buildWeekCard(WeeklyMealPlan plan, AppLocalizations l10n) {
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(
          borderRadius: BorderRadius.circular(12),
          side: BorderSide(
              color: isSelected ? colorPastelPink : Colors.transparent,
              width: 2)),
      child: Theme(
  ...
  Widget _buildDayItem(
      WeeklyMealPlan plan, DailyMealItem day, AppLocalizations l10n) {
                ...
                children: [
                  IconButton(
                    icon: const Icon(Icons.autorenew,
                        size: 20, color: AppDesign.petPink),
  ...
  Future<void> _deleteDay(WeeklyMealPlan plan, DailyMealItem day) async {
    ...
    final confirm = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
              backgroundColor: const Color(0xFF1E1E1E),
              content: Text(l10n.petMenuDeleteDayConfirm,
                  style: const TextStyle(color: Colors.white)),
              actions: [
                TextButton(
                    onPressed: () => Navigator.pop(ctx, false),
  ...
  Future<void> _editDayDialog(WeeklyMealPlan plan, DailyMealItem day) async {
    ...
    final changed = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
              backgroundColor: const Color(0xFF1E1E1E),
              title: Text(l10n.petMenuEditDayTitle,
                  style: const TextStyle(color: Colors.white)),
    ...
    if (mounted) {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(l10n.petMenuSaveSuccess),
          backgroundColor: AppDesign.success));
    }
  }
  ...
  Future<void> _recycleMeal(WeeklyMealPlan plan, DailyMealItem day) async {
    ...
    try {
      ...
      final newItem =
          await ref.read(petMenuGeneratorProvider).regenerateSingleMeal(
                day,
                widget.petName,
      ...
      if (newItem != null) {
        ...
        if (mounted) {
          ...
          if (!mounted) return;
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text("Opção alterada com sucesso!"),
              backgroundColor: AppDesign.success));
        }
      } else {
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text("Falha ao regenerar opção."),
              backgroundColor: AppDesign.error));
        }
      }
    } catch (e) {
  ...
  String _getDietLabel(String code, AppLocalizations l10n) {
    if (code.toLowerCase().startsWith("outra") ||
        code.toLowerCase().startsWith("other")) {
      return code;
    }
    switch (code) {
      case 'renal':
        return l10n.dietRenal ?? 'Renal';
      case 'hepatic':
        return l10n.dietHepatic ?? 'Hepatic';
      case 'gastrointestinal':
        return l10n.dietGastrointestinal ?? 'Gastrointestinal';
      case 'hypoallergenic':
        return l10n.dietHypoallergenic ?? 'Hypoallergenic';
      case 'obesity':
        return l10n.dietObesity ?? 'Obesity';
      case 'diabetes':
        return l10n.dietDiabetes ?? 'Diabetes';
      case 'cardiac':
        return l10n.dietCardiac ?? 'Cardiac';
      case 'urinary':
        return l10n.dietUrinary ?? 'Urinary';
      case 'muscle_gain':
        return l10n.dietMuscleGain ?? 'Muscle Gain';
      case 'pediatric':
        return l10n.dietPediatric ?? 'Pediatric';
      case 'growth':
        return l10n.dietGrowth ?? 'Growth';
      default:
        return code;
    }
  }
