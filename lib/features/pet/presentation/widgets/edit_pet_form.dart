...
import 'report_selector_modal.dart';
...
import 'profile_fragments/walks_fragment.dart'; // 🛡️ V300: Passeios Tab
...
class _EditPetFormState extends State<EditPetForm>
  ...
  List<Map<String, dynamic>> _woundHistory =
      []; // Local state for wound analysis history
  ...
  final TextEditingController _observacoesPlanosController =
  ...
  List<AnaliseFeridaModel> _historicoAnaliseFeridas =
      []; // 🛡️ Load structured history
  ...
  Future<void> _saveNow({bool silent = false}) async {
    ...
    try {
      ...
      final profile = PetProfileExtended(
        ...
        idadeExata: _idadeController.text.trim().isEmpty
            ? null
            : _idadeController.text.trim(),
        pesoAtual:
            double.tryParse(_pesoController.text.trim().replaceAll(',', '.')),
        pesoIdeal: double.tryParse(
            _pesoIdealController.text.trim().replaceAll(',', '.')),
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(
                AppLocalizations.of(context)!.commonSyncError(e.toString())),
  ...
  void initState() {
    ...
    debugPrint(
        '🛠️ [V_DEBUG] EditPetForm: Initializing with ID=$_petId (Name=${existing?.petName ?? "New Pet"})');
          ...
          if (rFile.existsSync()) {
            ...
            if (mounted) {
              setState(() {
                _profileImage = rFile;
                _initialImagePath =
                    rPath; // Update baseline to avoid duplicate save
              });
            }
          }
        }).catchError((e) {
    ...
    _microchipController =
        TextEditingController(text: existing?.microchip ?? '');
    ...
    if (existing != null) {
      ...
      _partnerNotes = Map.from(existing.partnerNotes)
          .map((k, v) => MapEntry(k, List<Map<String, dynamic>>.from(v)));
      _weightHistory = List.from(existing.weightHistory);
      _labExams =
          (existing.labExams).map((json) => LabExam.fromJson(json)).toList();
      _woundHistory = (existing.woundAnalysisHistory)
          .map((e) => Map<String, dynamic>.from(e))
          .toList();
      _historicoAnaliseFeridas =
          List.from(existing.historicoAnaliseFeridas); // 🛡️ Load
      _travelPreferences =
          Map<String, dynamic>.from(existing.travelPreferences); // 🛡️ V250
  ...
  void _updateProfileFromAI(Map<String, dynamic> result) {
    ...
    if (result.containsKey('identification')) {
      ...
      if (result.containsKey('metadata')) {
        setState(
            () => _reliability = result['metadata']['reliability']?.toString());
      }
  ...
  List<String> _getLocalizedItems(
      List<String> Function(AppLocalizations) getter) {
    if (!_depsReady) {
  ...
  void _onNameChanged() {
    debugPrint('DEBUG: Gravando [NOME] no disco agora...');
    _onUserTyping();
  }

  void _onRacaChanged() {
    debugPrint('DEBUG: Gravando [RAÇA] no disco agora...');
    _onUserTyping();
  }

  void _onIdadeChanged() {
    debugPrint('DEBUG: Gravando [IDADE] no disco agora...');
    _onUserTyping();
  }

  void _onPesoChanged() {
    debugPrint('DEBUG: Gravando [PESO] no disco agora...');
    _onUserTyping();
  }

  void _onPesoIdealChanged() {
    debugPrint('DEBUG: Gravando [PESO IDEAL] no disco agora...');
    _onUserTyping();
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.paused ||
      ...
      if (_hasChanges) {
        debugPrint(
            '⚠️ CRITICAL: Unsaved changes detected! Saving immediately...');
        _saveNow(silent: true);
      }
    ...
    return Stack(children: [
      Scaffold(
        ...
        appBar: AppBar(
          ...
          leading: IconButton(
            icon:
                const Icon(Icons.arrow_back, color: AppDesign.textPrimaryDark),
          ...
          bottom: TabBar(
            ...
            tabs: [
              ...
              Tab(
                  text:
                      AppLocalizations.of(context)!.petTravelTitle), // 🛡️ V250
              Tab(
                  text: AppLocalizations.of(context)!
                      .tabScanWalk), // 🛡️ V300: Passeios
            ],
          ),
        ),
        body: Column(children: [
          ...
          Expanded(
            ...
            children: [
              ...
              widget.existingProfile != null
                  ? WalksFragment(petProfile: widget.existingProfile!)
                  : const Center(
                      child: Text("Salve o pet para ver os passeios",
                          style: TextStyle(color: Colors.white24))),
            ],
          ))
        ]),
        bottomNavigationBar:
            null, // Reactive Auto-Save: No manual button needed
      ),
      ...
      if (mounted) {
        setState(() {
          ...
          _linkedPartnerModels =
              all.where((p) => _linkedPartnerIds.contains(p.id)).toList();
        });

        debugPrint(
            '✅ Loaded ${all.length} available partners, ${_linkedPartnerModels.length} linked');
      }
    } catch (e) {
  ...
  Future<void> _loadMealPlanStatus() async {
    final petName =
        widget.existingProfile?.petName ?? _nameController.text.trim();
  ...
  bool _isGallery(String n) =>
      n.startsWith('gallery_') ||
      n.startsWith('profile_pic') ||
      n.startsWith('health_analysis_');
  ...
  Future<void> _loadAttachments() async {
    ...
    setState(() {
      _attachments['identity'] = _optimizeList(
          allDocs.where((f) => _isIdentity(path.basename(f.path))).toList());
      _attachments['health_exams'] = _optimizeList(
          allDocs.where((f) => _isExam(path.basename(f.path))).toList());
      _attachments['health_prescriptions'] = _optimizeList(allDocs
          .where((f) => _isPrescription(path.basename(f.path)))
          .toList());
      _attachments['health_vaccines'] = _optimizeList(
          allDocs.where((f) => _isVaccine(path.basename(f.path))).toList());
      _attachments['nutrition'] = _optimizeList(
          allDocs.where((f) => _isNutrition(path.basename(f.path))).toList());
      _attachments['travel_docs'] = _optimizeList(
          allDocs.where((f) => _isTravel(path.basename(f.path))).toList());

      // 🛡️ V_FIX: Specific Travel Categories
      _attachments['travel_health_cert'] = _optimizeList(allDocs
          .where((f) => path.basename(f.path).startsWith('travel_health_cert_'))
          .toList());
      _attachments['travel_vaccination_card'] = _optimizeList(allDocs
          .where((f) =>
              path.basename(f.path).startsWith('travel_vaccination_card_'))
          .toList());
      _attachments['travel_microchip_cert'] = _optimizeList(allDocs
          .where(
              (f) => path.basename(f.path).startsWith('travel_microchip_cert_'))
          .toList());
      _attachments['travel_crate_id'] = _optimizeList(allDocs
          .where((f) => path.basename(f.path).startsWith('travel_crate_id_'))
          .toList());
      _attachments['travel_leish_vax'] = _optimizeList(allDocs
          .where((f) => path.basename(f.path).startsWith('travel_leish_vax_'))
          .toList());
      _attachments['travel_felv_test'] = _optimizeList(allDocs
          .where((f) => path.basename(f.path).startsWith('travel_felv_test_'))
          .toList());

      // Gallery gets the rest (Catch-all)
      _attachments['gallery'] = _optimizeList(
          allDocs.where((f) => _isGallery(path.basename(f.path))).toList());

      // 🛡️ V123: Debug logging
      debugPrint('📎 [V123] Attachments loaded:');
      debugPrint('  Identity: ${_attachments['identity']?.length ?? 0}');
      debugPrint(
          '  Prescriptions: ${_attachments['health_prescriptions']?.length ?? 0}');
      ...
      if (allDocs.isNotEmpty) {
        ...
        for (var doc in allDocs.take(5)) {
          ...
          debugPrint('  - $name');
          debugPrint(
              '    Identity: ${_isIdentity(name)}, Prescription: ${_isPrescription(name)}, Vaccine: ${_isVaccine(name)}');
        }
      }
    });
  }
  ...
  Future<void> _analyzePetProfile(File file) async {
    ...
    try {
      ...
      String? knownSpecies =
          (_especie != null && _especie!.isNotEmpty) ? _especie : null;
      String? knownBreed = _racaController.text.trim().isNotEmpty
      ...
      if (_tabController.index == 1) {
        ...
        debugPrint(
            '🛡️ [EditPetForm] Health Tab (1) active. Switching to DIAGNOSIS mode.');
      } else if (_tabController.index == 2) {
        // Could enable food analysis here if implemented for Pet Food
        // mode = ScannutMode.petNutrition; // Future
      }

      final result = await _visionService.analisarFotoPet(file, locale,
          knownSpecies: knownSpecies, knownBreed: knownBreed, mode: mode);
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Column(
              ...
              children: [
                Text(AppLocalizations.of(context)!.auto_fill_success,
                    style: const TextStyle(fontWeight: FontWeight.bold)),
                Text(AppLocalizations.of(context)!.is_pet_breed_correct(
                    result['identification']?['breed']?.toString() ?? 'SRD')),
              ],
            ),
            backgroundColor: AppDesign.petPink,
  ...
  Future<void> _handleNewImageSelection(File tempFile) async {
    ...
    if (petName.isEmpty) {
      petName = "New_Pet_${DateTime.now().millisecondsSinceEpoch}";
    }

    if (isHealthMode) {
      debugPrint(
          '🛡️ [Context-Aware] Health Tab detected. Processing as CLINICAL ANALYSIS (No Profile Pic Change).');
      try {
        ...
        if (permanentPath != null && mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text(
                'Analisando imagem clínica... A foto de perfil não será alterada.'),
            backgroundColor: AppDesign.petPink,
            ...
            child: Column(
              ...
              children: [
                ...
                ListTile(
                  ...
                  title: Text(AppLocalizations.of(context)!.petChooseGallery,
                      style: const TextStyle(color: AppDesign.textPrimaryDark)),
                  onTap: () async {
                    Navigator.pop(ctx);
  ...
  Future<void> _addAttachment(String type) async {
    ...
    if (petName.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(AppLocalizations.of(context)!.commonSaveNameFirst)));
      return;
    }
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                Text(
                    isGallery
                        ? AppLocalizations.of(context)!.petAddMedia
                        : AppLocalizations.of(context)!.petAttachDoc,
                    style: GoogleFonts.poppins(
                        color: AppDesign.textPrimaryDark, fontSize: 16)),
                const SizedBox(height: 20),
                ListTile(
                  leading: const Icon(Icons.camera_alt, color: AppDesign.info),
                  title: Text(AppLocalizations.of(context)!.petCameraPhoto,
                      style: const TextStyle(color: AppDesign.textPrimaryDark)),
                  onTap: () async {
                    Navigator.pop(ctx);
                    final file = isTravel
                        ? await _fileService.pickAndOptimizeImage(
                ...
                ListTile(
                  leading: const Icon(Icons.image, color: AppDesign.petPink),
                  title: Text(AppLocalizations.of(context)!.petGalleryPhoto,
                      style: const TextStyle(color: AppDesign.textPrimaryDark)),
                  onTap: () async {
                    Navigator.pop(ctx);
                    final file = isTravel
                ...
                if (isGallery) ...[
                  ListTile(
                    leading:
                        const Icon(Icons.videocam, color: AppDesign.warning),
                    title: Text(AppLocalizations.of(context)!.petCameraVideo,
                        style:
                  ...
                  ListTile(
                    ...
                    title: Text(AppLocalizations.of(context)!.petGalleryVideo,
                        style:
                            const TextStyle(color: AppDesign.textPrimaryDark)),
                    onTap: () async {
                      Navigator.pop(ctx);
                  ...
                  ListTile(
                    ...
                    title: const Text('PDF',
                        style: TextStyle(color: AppDesign.textPrimaryDark)),
                    onTap: () async {
                      Navigator.pop(ctx);
                      try {
                        debugPrint(
                            '🔍 [PDF_ATTACH] Starting PDF picker for type: $type');
                        final file = await _fileService.pickPdfFile();

                        if (file != null) {
                          debugPrint(
                              '✅ [PDF_ATTACH] File picked: ${file.path}');
                          await _saveFile(file, petName, type);
                        } else {
                          debugPrint(
                              '⚠️ [PDF_ATTACH] File picker returned null (user cancelled or error)');
                          if (mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                    content:
  ...
  Future<void> _saveFile(File file, String petName, String type) async {
    try {
      debugPrint(
          '💾 [PDF_SAVE] Saving file: ${file.path} for pet: $petName, type: $type');
      ...
      if (singleDocTypes.contains(type)) {
        ...
        debugPrint(
            '🧹 [V_FIX] Cleared ${existingFiles.length} old files for single-doc category: $type');
      }
      ...
      if (savedPath != null) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: Text(AppLocalizations.of(context)!.petDocAttached)));
        }
      } else {
  ...
  Future<void> _deleteAttachment(File file) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text(AppLocalizations.of(context)!.petDeleteAttachment,
            style: const TextStyle(color: Colors.white)),
        content: Text(AppLocalizations.of(context)!.petDeleteAttachmentContent,
  ...
  Widget _buildIdentityTabContent() {
    return IdentityFragment(
      ...
      onNivelAtividadeChanged: (val) {
        setState(() => _nivelAtividade = val);
        _onUserInteractionGeneric();
      },
      onStatusReprodutivoChanged: (val) {
        setState(() => _statusReprodutivo = val);
        _onUserInteractionGeneric();
      },
      onObservacoesChanged: (val) {
        setState(() => _observacoesIdentidade = val);
        _onUserTyping();
      },
  ...
  Widget _buildHealthTabContent() {
    return HealthFragment(
      ...
      dataUltimaAntirrabica: _dataUltimaAntirrabica,
      frequenciaBanho: _frequenciaBanho,
      bathOptions: _getLocalizedItems(
          (l) => [l.petBathBiweekly, l.petBathWeekly, l.petBathMonthly]),
      petId: _petId ?? _nameController.text.trim(),
      species: _especie ?? '',
      labExams: _labExams,
      observacoesSaude: _observacoesSaude,
      onV10DateSelected: (date) {
        setState(() => _dataUltimaV10 = date);
        _onUserInteractionGeneric();
      },
      onAntirrabicaDateSelected: (date) {
        setState(() => _dataUltimaAntirrabica = date);
        _onUserInteractionGeneric();
      },
      onFrequenciaBanhoChanged: (val) {
        setState(() => _frequenciaBanho = val);
        _onUserInteractionGeneric();
      },
  ...
  Future<void> _handleDeleteAnalysis(Map<String, dynamic> item) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
            'Tem certeza que deseja remover esta análise permanentemente?',
    ...
    setState(() {
      _analysisHistory
          .removeWhere((a) => a['last_updated'] == item['last_updated']);
    });
    _onUserInteractionGeneric();
  }
  ...
  Widget _buildPlansTabContent() {
    return PlansFragment(
      ...
      onHealthPlanChanged: (val) {
        setState(() => _healthPlan = val);
        _onUserInteractionGeneric();
      },
      onAssistancePlanChanged: (val) {
        setState(() => _assistancePlan = val);
        _onUserInteractionGeneric();
      },
      onFuneralPlanChanged: (val) {
        setState(() => _funeralPlan = val);
        _onUserInteractionGeneric();
      },
      onLifeInsuranceChanged: (val) {
        setState(() => _lifeInsurance = val);
        _onUserInteractionGeneric();
      },
  ...
  Future<void> _confirmDelete() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text(AppLocalizations.of(context)!.petDeleteTitle,
            style: const TextStyle(color: Colors.white)),
        content: Text(
          AppLocalizations.of(context)!.petDeleteContent(
              widget.existingProfile?.petName ??
                  AppLocalizations.of(context)!.petDefaultName),
          style: const TextStyle(color: Colors.white70),
        ),
        actions: [
          TextButton(
          ...
          TextButton(
            ...
            child: Text(AppLocalizations.of(context)!.petDeleteConfirm,
      ...
      return Card(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              _buildSectionTitle(
                  AppLocalizations.of(context)!.petDiseaseHistory),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              AppLocalizations.of(context)!.petWoundsCount(
                  woundHistory.length + structuredHistory.length),
            ...
            ...woundHistory.where((legacy) {
              ...
              return !structuredHistory
                  .any((structured) => structured.imagemRef == legacyPath);
            }).map((analysis) => _buildWoundAnalysisCard(analysis)),

            // Render New Structured History (Higher precision)
            ...structuredHistory
                .map((analysis) => _buildStructuredWoundCard(analysis))
                .toList()
                .reversed,
          ],
        ),
  ...
  Widget _buildWoundAnalysisCard(Map<String, dynamic> analysis) {
    ...
    final diagnosis = (diagnosisRaw != null && diagnosisRaw.isNotEmpty)
        ? diagnosisRaw
        : AppLocalizations.of(context)!.diagnosisPending;
    ...
    } else if (sevLower.contains('média') ||
        sevLower.contains('media') ||
        sevLower.contains('medium')) {
    ...
    return Container(
      ...
      child: ExpansionTile(
        ...
        trailing: IconButton(
          ...
          onPressed: () =>
              _confirmDeleteWoundAnalysis(analysis['date'] as String),
    ...
    try {
      ...
      if (freshData != null && freshData['data'] != null) {
        ...
        if (data['labExams'] != null) {
          try {
            final list = (data['labExams'] as List)
                .map((e) => LabExam.fromJson(Map<String, dynamic>.from(e)))
                .toList();
        ...
        if (data['linked_partner_ids'] != null) {
          if (mounted) {
            setState(() {
              _linkedPartnerIds =
                  (data['linked_partner_ids'] as List?)?.cast<String>() ?? [];
            });
        ...
        if (mounted) {
          setState(() {
            ...
            _observacoesPlanosController.text =
                data['observacoes_planos'] ?? '';
          });
        }
        ...
        decoration: InputDecoration(
          label: RichText(
              text: TextSpan(
                ...
                if (isRequired)
                  const TextSpan(
                      text: ' *', style: TextStyle(color: AppDesign.petPink)),
              ])),
  ...
  Future<void> _toggleListening(TextEditingController controller) async {
    ...
    final savedCursor = controller.selection.baseOffset < 0
    ...
    await _speech.listen(
      onResult: (result) {
        ...
        String textAfter =
            controller.text.substring(savedCursor + _lastWords.length);

        if (textAfter.length + savedCursor + _lastWords.length >
            controller.text.length) {
        ...
        controller.value = TextEditingValue(
          text: newText,
          selection:
              TextSelection.collapsed(offset: savedCursor + recognized.length),
        );
          ...
          children: options.map((option) {
            ...
            return ChoiceChip(
              ...
              shape: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(12),
                side: BorderSide(
                    color: isSelected ? AppDesign.petPink : Colors.white12),
              ),
              showCheckmark: false,
            );
          }).toList(),
          ...
          children: [
            ...
            IconButton(
              ...
              icon: const Icon(Icons.add_circle,
                  color: AppDesign.petPink, size: 32),
            ),
          ],
        ),
        const SizedBox(height: 12),
  ...
  Future<void> _generateNewMenu(WidgetRef ref) async {
    if (_nameController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(AppLocalizations.of(context)!.petNameRequired)));
      return;
    }
    ...
    try {
      ...
      final plans =
          await MealPlanService().getPlansForPet(_nameController.text.trim());
      if (plans.isNotEmpty) {
        ...
        setState(() {
          ...
          _currentRawAnalysis!['plano_semanal'] = allMeals
              .map((m) => {
                    'dia': m.dayOfWeek,
                    'hora': m.time,
                    'titulo': m.title,
        ...
        child: Column(
          children: [
            const Icon(Icons.restaurant_menu,
                size: 48, color: AppDesign.petPink),
            const SizedBox(height: 16),
            Text(
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text(AppLocalizations.of(context)!.petViewMenu ??
                    'Ver Cardápio'),
                style: ElevatedButton.styleFrom(
                  ...
                  Navigator.push(
                      ...
                      MaterialPageRoute(
                          builder: (_) => WeeklyMenuScreen(
                                ...
                                generalGuidelines: '',
                                petName: widget.existingProfile?.petName ??
            ...
            Consumer(builder: (context, ref, child) {
              return SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label:
                      Text(AppLocalizations.of(context)!.petGenerateWeeklyMenu),
                  style: ElevatedButton.styleFrom(
                    backgroundColor: AppDesign.petPink,
  ...
  Future<void> _explainLabExam(String examId) async {
    ...
    try {
      ...
      if (currentExam.extractedText == null ||
          currentExam.extractedText!.isEmpty) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: Text(AppLocalizations.of(context)!
                  .petNoDocumentsAttached), // Using generic "No docs/text" msg or similar
            ...
            showDialog(
                ...
                builder: (_) => AlertDialog(
                      title: const Text('Texto não detectado',
                          style: TextStyle(
                              color: Colors
                                  .white)), // Hardcoded fallback or use l10n
                      backgroundColor: Colors.grey[900],
                      content: const Text(
                          'Não foi possível ler texto nesta imagem. Se for uma foto de fezes ou lesão para análise visual, utilize a aba "Análises" ou "Feridas". Esta aba ("Exames") é exclusiva para laudos e documentos de texto.',
                          style: TextStyle(color: Colors.white70)),
                      actions: [
                        TextButton(
      ...
      if (locale.startsWith('en')) {
        ...
        languageInstruction =
            "Respond in English. Translate all medical terms.";
      } else if (locale.startsWith('es')) {
        languageName = "Spanish";
        languageInstruction =
            "Responda en Español. Traduzca todos los términos médicos.";
      }

      // 2. Run AI Explanation
      final explanation = await _labExamService.generateExplanation(currentExam,
          languageName: languageName, languageInstruction: languageInstruction);

      if (mounted) {
        setState(() {
          _labExams[examIndex] = currentExam.copyWith(
              aiExplanation: explanation, isProcessing: false);
        });
        _onUserInteractionGeneric();
      }
    } catch (e) {
      ...
      if (mounted) {
        setState(() {
          _labExams[examIndex] =
              _labExams[examIndex].copyWith(isProcessing: false);
        });

        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(AppLocalizations.of(context)!
                .errorGeneratingExplanation(e.toString())),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  void _deleteLabExam(String examId) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: Colors.grey[900],
        title: Text(AppLocalizations.of(context)!.examDeleteTitle,
  ...
  PetProfileExtended _deepCopyProfile(PetProfileExtended original) {
    return PetProfileExtended(
      ...
      partnerNotes: Map<String, List<Map<String, dynamic>>>.from(original
          .partnerNotes
          .map((k, v) => MapEntry(k, List<Map<String, dynamic>>.from(v)))),
      ...
      rawAnalysis: original.rawAnalysis != null
          ? Map<String, dynamic>.from(original.rawAnalysis!)
          : null,
      travelPreferences:
          Map<String, dynamic>.from(original.travelPreferences), // 🛡️ V250
    );
  }
    ...
    final result = await Navigator.push(
        ...
        MaterialPageRoute(
            builder: (_) => PartnersHubScreen(
                  ...
                  petId: widget.existingProfile?.petName ??
  ...
  Future<void> _generatePetReport({AnaliseFeridaModel? specificWound}) async {
    try {
      ...
      debugPrint(
          '[PDF_FULL] Generating complete report for ${_nameController.text.trim()}');
      ...
      try {
        ...
        if (petData != null && petData is Map) {
          ...
          if (rawData.containsKey('rawAnalysis') &&
              rawData['rawAnalysis'] != null) {
            ...
            if (analysis.containsKey('plano_semanal')) {
              ...
              debugPrint(
                  '✅ [V73] Meal plan extracted from rawAnalysis: ${analysis['plano_semanal']?.length ?? 0} days');
            } else {
      ...
      try {
        ...
        final petPlans = mealPlanBox.values
            .where((plan) => plan.petId == _nameController.text.trim())
            .toList();
        ...
        if (petPlans.isNotEmpty) {
          ...
          debugPrint(
              '✅ [V74] Found ${petPlans.length} meal plan(s) in weekly_meal_plans box');
          debugPrint(
              '✅ [V74] Using latest plan: ${latestPlan.id} (${latestPlan.startDate} to ${latestPlan.endDate})');

          // Convert WeeklyMealPlan to rawAnalysis format if not already present
          if (mealPlanData == null ||
              !mealPlanData.containsKey('plano_semanal')) {
            ...
            debugPrint(
                '✅ [V74] Converted WeeklyMealPlan to rawAnalysis format: ${planoSemanal.length} days');
          }
        } else {
          debugPrint(
              '⚠️ [V74] No meal plans found in weekly_meal_plans box for pet: ${_nameController.text.trim()}');
        }
      } catch (e) {
      ...
      if (finalRawAnalysis != null &&
          finalRawAnalysis.containsKey('plano_semanal')) {
        debugPrint(
            '🟢 [V74] MEAL PLAN READY FOR PDF: ${finalRawAnalysis['plano_semanal']?.length ?? 0} days');
      } else {
        debugPrint(
      ...
      final profile = PetProfileExtended(
        ...
        historicoAnaliseFeridas:
            _historicoAnaliseFeridas, // 🛡️ V_FIX: Pass structured history
        analysisHistory: _analysisHistory,
        observacoesIdentidade: _observacoesIdentidade,
        observacoesSaude: _observacoesSaude,
        observacoesNutricao: _observacoesNutricao,
        ...
        microchip: _microchipController.text
            .trim(), // 🛡️ V_FIX: ADD MISSING MICROCHIP
        lastUpdated: DateTime.now(),
      ...
      if (specificWound == null) {
        // 🛡️ [V_MEGA_REPORT] Redirect to Selector Modal
        if (mounted) {
          ReportSelectorModal.show(context, profile);
          return;
        }
      }
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) => PdfPreviewScreen(
            title:
                '${AppLocalizations.of(context)!.pdfReportTitle} - ${profile.petName}',
            buildPdf: (format) async {
              if (specificWound != null) {
                final pdf =
                    await PetPdfGenerator().generateSingleAnalysisReport(
                  profile: profile,
                  strings: AppLocalizations.of(context)!,
              ...
              try {
                ...
                final vaccineEvents =
                    events.where((e) => e.type == EventType.vaccine).toList();
  ...
  String _localizeValue(String? value, {bool isReliability = false}) {
    ...
    if (v == 'não informado' ||
        ...
        v == 'nenhum' ||
        v == 'ninguno') {
      return strings.petNotOffice;
    }
    if (v == 'sem diagnóstico' || v == 'no diagnosis' || v == 'sin diagnóstico') {
      return strings.petDiagnosisDefault;
    }

    // Partner Categories
    if (v == 'todos' || v == 'all') return strings.partnersFilterAll;
    if (v.contains('veterinário') ||
        v.contains('veterinário') ||
        v.contains('veterinarian')) {
      return strings.partnersFilterVet;
    }
    if (v.contains('pet shop')) return strings.partnersFilterPetShop;
    if (v.contains('farmácia') || v.contains('pharmacy')) {
    ...
    if (v.contains('hotel') || v.contains('hotéis')) {
      return strings.partnersFilterHotel;
    }
    if (v.contains('lab') ||
        v.contains('laboratório') ||
        v.contains('laboratory')) {
      return strings.partnersFilterLab;
    }

    if (isReliability) {
      if (v.contains('baixa') || v.contains('low') || v.contains('baja')) {
        return strings.petReliabilityLow;
      }
      if (v.contains('média') || v.contains('media') || v.contains('medium')) {
        return strings.petReliabilityMedium;
      }
      if (v.contains('alta') || v.contains('high')) {
        return strings.petReliabilityHigh;
      }
    }

    // Activity Levels
    if (v.contains('sedentário') || v.contains('baixo') || v.contains('low')) {
      return strings.petActivityLow;
    }
    if (v.contains('moderado') ||
        v.contains('moderate') ||
        v.contains('médio') ||
        v.contains('meio')) {
    ...
    if (v.contains('atleta') || v.contains('athlete')) {
      return strings.petActivityAthlete;
    }

    // Reproductive Status
    if (v.contains('castrado') || v.contains('neutered')) {
      return strings.petNeutered;
    }
    if (v.contains('inteiro') ||
        v.contains('intacto') ||
        v.contains('intact') ||
        v.contains('entero')) {
    ...
    // Bath Frequency
    if (v.contains('semanal') || v.contains('weekly')) {
      return strings.petBathWeekly;
    }
    if (v.contains('quinzenal') ||
        v.contains('biweekly') ||
        v.contains('bi-weekly')) {
      return strings.petBathBiweekly;
    }
    if (v.contains('mensal') || v.contains('monthly')) {
      return strings.petBathMonthly;
    }

    // General Not Identified/Estimated/Informed
    if (v.contains('não identificada') ||
        v.contains('not identified') ||
        v.contains('no identificado')) {
      return strings.petNotIdentified;
    }
    if (v.contains('não estimada') ||
        v.contains('not estimated') ||
        v.contains('no estimado')) {
      return strings.petNotEstimated;
    }

    // Model Fallbacks
    if (v.contains('consulte veterinário') ||
        v.contains('consult a veterinarian') ||
        v.contains('consulte al veterinario')) {
      return strings.petConsultVet;
    }
    if (v.contains('hemograma') || v.contains('blood count')) {
      return strings.petHemogramaCheckup;
    }
    if (v.contains('reforço positivo') ||
        v.contains('positive reinforcement') ||
        v.contains('refuerzo positivo')) {
      return strings.petPositiveReinforcement;
    }
    if (v.contains('brinquedos interativos') ||
        v.contains('interactive toys') ||
        v.contains('juguetes interactivos')) {
      return strings.petInteractiveToys;
    }
    if (v.contains('consulte um vet') ||
        v.contains('consult a vet') ||
        v.contains('consulte a un vet')) {
      return strings.petConsultVetCare;
    }

    // Size
    if (v.contains('pequeno') || v.contains('small')) {
      return strings.petSizeSmall;
    }
    if (v.contains('médio') || v.contains('medium') || v.contains('mediano')) {
      return strings.petSizeMedium;
    }
    if (v.contains('grande') || v.contains('large')) {
      return strings.petSizeLarge;
    }
    if (v.contains('gigante') || v.contains('giant')) {
      return strings.petSizeGiant;
    }

    // Coat Type
    if (v.contains('curto') || v.contains('short') || v.contains('corto')) {
      return strings.petCoatShort;
    }
    if (v.contains('longo') || v.contains('long') || v.contains('largo')) {
      return strings.petCoatLong;
    }
    if (v.contains('duplo') || v.contains('double') || v.contains('doble')) {
      return strings.petCoatDouble;
    }
    if (v.contains('duro') || v.contains('wire')) return strings.petCoatWire;
    if (v.contains('encaracolado') ||
        v.contains('curly') ||
        v.contains('rizado')) {
      return strings.petCoatCurly;
    }
  ...
  Widget _buildProfileHeader() {
    return SliverAppBar(
      ...
      leading: widget.onCancel != null
          ...
          : IconButton(
              icon: const Icon(Icons.arrow_back, color: AppDesign.petPink),
              onPressed: () => Navigator.pop(context)),
      actions: [
        PdfActionButton(onPressed: _generatePetReport),
  ...
  Widget _buildStructuredWoundCard(AnaliseFeridaModel analysis) {
    ...
    if (risk.contains('red') ||
        risk.contains('vermelho') ||
        risk.contains('alta')) {
      severityColor = AppDesign.error;
    } else if (risk.contains('yellow') ||
        risk.contains('amarelo') ||
        risk.contains('média')) {
      severityColor = Colors.orange;
    } else {
    ...
    final bool isStool =
        analysis.categoria == 'fezes' || analysis.categoria == 'stool';
    ...
    if (isStool && analysis.achadosVisuais.containsKey('bristol_scale')) {
      title =
          'Bristol ${analysis.achadosVisuais['bristol_scale']} - ${analysis.achadosVisuais['color_name'] ?? 'Cor N/A'}';
    }
  ...
  void _confirmDeleteStructuredWoundAnalysis(
      AnaliseFeridaModel analysis) async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text(AppLocalizations.of(context)!.commonDelete),
        content: Text(
            '${AppLocalizations.of(context)!.commonDelete} a análise de ${DateFormat.yMd().format(analysis.dataAnalise)}?'),
        actions: [
          TextButton(
              onPressed: () => Navigator.pop(ctx, false),
    ...
    if (confirm == true) {
      setState(() {
        ...
        _woundHistory.removeWhere((w) =>
            w['date'] == analysis.dataAnalise.toIso8601String() ||
            w['imagePath'] == analysis.imagemRef);
        _markDirty();
      });
      _onUserInteractionGeneric();
    }
            ...
            child: SingleChildScrollView(
              ...
              if (File(analysis.imagemRef).existsSync())
                Container(
                  ...
                  child: Image.file(File(analysis.imagemRef),
              ...
              Padding(
                ...
                child: Column(
                    ...
                    children: [
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                    (analysis.categoria == 'fezes' ||
                                            analysis.categoria == 'stool')
                                        ? 'Análise de Fezes'
                      ...
                      const SizedBox(height: 16),
                      _buildDetailRow('Nível de Risco', analysis.nivelRisco,
                          color: Colors.white),
                      _buildDetailRow(
                          'Data',
                          DateFormat('dd/MM/yyyy HH:mm')
                              .format(analysis.dataAnalise),
                          color: Colors.white),
                      if (analysis.categoria == 'fezes' ||
                          analysis.categoria == 'stool') ...[
                        if (analysis.achadosVisuais.containsKey('bristol') ||
                            analysis.achadosVisuais
                                .containsKey('Escala de Bristol'))
                          _buildDetailRow(
                              'Escala de Bristol',
                              analysis.achadosVisuais['bristol']?.toString() ??
                                  analysis.achadosVisuais['Escala de Bristol']
                                      ?.toString() ??
                                  'N/A'),
                        if (analysis.achadosVisuais.containsKey('color') ||
                            analysis.achadosVisuais.containsKey('Cor'))
                          _buildDetailRow(
                              'Cor',
                              analysis.achadosVisuais['color']?.toString() ??
                                  analysis.achadosVisuais['Cor']?.toString() ??
                                  'N/A'),
                      ],
                      ...
                      if (analysis.diagnosticosProvaveis.isNotEmpty) ...[
                        ...
                        const SizedBox(height: 4),
                        ...analysis.diagnosticosProvaveis.map((c) => Padding(
                      ...
                      if (analysis.descricaoVisual != null) ...[
                        const Text('Descrição Visual:',
                            style:
                                TextStyle(color: Colors.white60, fontSize: 12)),
                        const SizedBox(height: 4),
                        Text(analysis.descricaoVisual!,
                            style: const TextStyle(color: Colors.white)),
                        const SizedBox(height: 12),
                      ],
  ...
  Widget _buildPartnersTabContent() {
    ...
    return PartnersFragment(
      ...
      selectedPartnerFilter:
          _partnerFilter.isEmpty ? l10n.partnersFilterAll : _partnerFilter,
      ...
      onLinkStatusChanged: (p) => setState(() {
        if (_linkedPartnerIds.contains(p.id)) {
          ...
          PetIndexingService().indexPartnerInteraction(
            ...
            localizedNotes:
                'O parceiro ${p.name} (${p.category}) foi vinculado ao prontuário do pet.',
          );
        }
        _markDirty();
      }),
      onPartnerUpdated: (_) => _loadLinkedPartners(),
      onOpenAgenda: (p) => Navigator.push(
          context,
          MaterialPageRoute(
              builder: (_) => PartnerAgendaScreen(
                    partner: p,
                    initialEvents: const [],
                    petId:
                        widget.existingProfile?.petName ?? _nameController.text,
                    onSave: (list) {},
                  ))),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          shape: BoxShape.circle,
          border: Border.all(
              color: AppDesign.petPink.withValues(alpha: 0.5), width: 3),
        ),
        child: Stack(
          children: [
            Container(
              ...
              child: ClipOval(
                child: imageProvider != null
                    ...
                    : const Center(
                        child:
                            Icon(Icons.pets, size: 60, color: Colors.white24)),
              ),
            ),
            Positioned(
              bottom: 0,
  ...
  Future<void> _reloadAnalysisHistory() async {
    debugPrint(
        '🔄 [EditPetForm] ========== RELOAD ANALYSIS HISTORY ==========');
    try {
      final petName = _nameController.text.trim();
      ...
      if (mounted) {
        ...
        debugPrint(
            '   - History length: ${history is List ? history.length : 'N/A'}');
        ...
        setState(() {
          if (history is List) {
            ...
            debugPrint(
                '✅ [EditPetForm] _analysisHistory atualizado: ${_analysisHistory.length} items');
            ...
            if (_analysisHistory.isNotEmpty) {
              ...
              for (var i = 0; i < _analysisHistory.length && i < 3; i++) {
                ...
                debugPrint(
                    '   [$i] type: ${item['analysis_type']}, score: ${item['health_score']}');
              }
            }
          } else {
        ...
        debugPrint(
            '🔄 [EditPetForm] Analysis history reloaded: ${_analysisHistory.length} items');
      } else {
