...
class _PetEventHistoryScreenState extends State<PetEventHistoryScreen> {
        ...
        actions: [
          ...
          IconButton(
            icon: Icon(Icons.filter_list,
                color: (_selectedDateRange != null || _selectedType != null)
                    ? AppDesign.petPink
                    : Colors.white),
            tooltip: 'Filtrar',
      ...
      body: ValueListenableBuilder(
        ...
        builder: (context, box, _) {
          ...
          var events = PetEventRepository().listEventsByPet(
            ...
            to: _selectedDateRange?.end != null
  ...
  Widget _buildEventTimelineItem(
      BuildContext context, PetEventModel event, AppLocalizations l10n) {
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            Container(
                              ...
                              child: Icon(_getGroupIcon(event.group),
                                  color: AppDesign.petPink, size: 14),
                            ),
                              ...
                              children: [
                                Row(
                                  ...
                                  children: [
                                    ...
                                    if (event.group == 'schedule' ||
                                        event.data['is_task'] == true)
                                      GestureDetector(
                                        onTap: () async {
                                          final isCompleted =
                                              event.data['is_completed'] ==
                                                  true;
                                          if (isCompleted) {
                                            return; // Prevent double indexing
                                          }
                                          ...
                                          final updatedEvent =
                                              event.copyWith(data: updatedData);
                                          await PetEventRepository()
                                          ...
                                          try {
                                            PetIndexingService()
                                              ...
                                              taskId: event.id,
                                              localizedTitle: AppLocalizations
                                                      .of(context)!
                                                  .petIndexing_taskCompleted(
                                                      event.title),
                                            );
                                            ...
                                            debugPrint(
                                                'Error indexing task completion: $e');
                                          }
                                        },
                                        child: Padding(
                                          padding:
                        ...
                        if (event.notes.isNotEmpty ||
                            event.data['source'] == 'vocal_analysis') ...[
                          const SizedBox(height: 10),
                          Builder(builder: (context) {
                            if (event.data['source'] == 'vocal_analysis') {
                              final fname =
                                  event.data['file_name']?.toString() ??
                                      'Audio';
                              final displayFname = fname
                                      .startsWith('sound_rec_')
                                  ? (l10n.soundRecording.replaceAll('...', ''))
                                  : fname;

                              final reason = event.data['reason_simple'] ??
                                  event.data['reason'];
                              final action = event.data['action_tip'] ??
                                  event.data['action'];
                              ...
                              return Column(
                                ...
                                children: [
                                  Text(
                                      '${l10n.soundUploadBtn.split(' ').last}: $displayFname',
                                      style: const TextStyle(
                              ...
                              return [
                                ...
                                if (File(sidecarPath).existsSync())
                                  GestureDetector(
                                    onTap: () {
                                      try {
                                        final content = File(sidecarPath)
                                            .readAsStringSync();
                                        if (!mounted) return;
  ...
  Future<void> _confirmDelete(BuildContext context, String eventId) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: AppDesign.surfaceDark,
        title:
            const Text('Excluir Evento', style: TextStyle(color: Colors.white)),
        content: const Text('Deseja remover este registro do histórico?',
  ...
  void _handleDeepLink(
      BuildContext context, String deepLink, PetEventModel event) async {
      ...
      try {
        ...
        if (context.mounted) {
          Navigator.push(
              ...
              MaterialPageRoute(
                  builder: (_) =>
                      PartnerRegistrationScreen(initialData: partner)));
        }
      } catch (e) {
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
    ...
    if (deepLink.startsWith('scannut://pet/analysis/')) {
      ...
      if (event.attachments.isNotEmpty) {
        final firstImage = event.attachments.firstWhere(
            (a) => a.kind == 'image',
            orElse: () => event.attachments.first);
        final sidecarPath = path.join(path.dirname(firstImage.path),
        ...
        if (File(sidecarPath).existsSync()) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text('Resultado da análise não encontrado.')));
        }
      } else {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Detalhes desta análise não disponíveis offline.')));
      }
      return;
    }
          ...
          builder: (ctx) => AlertDialog(
                ...
                title: Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    Expanded(
                        child: Text('${l10n.soundAnalysisTitle}: $emotion',
                            style: const TextStyle(
  ...
  void _openAnalysisResult(
      BuildContext context, String jsonString, String imagePath) {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Não foi possível carregar os detalhes desta análise.'),
