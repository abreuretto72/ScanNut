...
class _PetResultScreenState extends ConsumerState<PetResultScreen> {
  ...
  Future<void> _analyzeImage() async {
    try {
        ...
        if (existing != null) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: Text(
                  AppLocalizations.of(context)!.error_image_already_analyzed),
      ...
      try {
        ...
        final petNameForPath = result.petName ??
        ...
        _permanentImage =
            widget.imageFile; // Fallback to cache (better than nothing)
      }

      // AUTO-SAVE LOGIC (Lei de Ferro: Persistência Real Imediata)
      if (!_autoSaveAttempted) {
        _autoSaveAttempted = true;
        debugPrint(
            '💾 [Lei de Ferro] Iniciando persistência obrigatória do pet...');
        await _performAutoSave(
            result); // Awaiting ensures it's on disk before UI release
        debugPrint('✅ [Lei de Ferro] Pet persistido com sucesso.');
      }
  ...
  void _generatePDF(BuildContext context, PetAnalysisResult? result) async {
    ...
    // 🛡️ V116: AUDIT LOG
    debugPrint(
        '📄 [V116-PDF] Preparando exportação PDF do Dossiê Veterinário 360 para ${result.petName ?? "Pet"}');
  ...
  void _handleAutoSaveAndNav(
      BuildContext context, PetAnalysisResult? result) async {
    if (result == null) return;
    ...
    await ps.init();
    final existingEntry =
        await ps.getProfile(result.petId ?? result.petName ?? '');
    ...
    if (existingEntry != null && existingEntry['data'] != null) {
      ...
      profile = PetProfileExtended.fromHiveEntry(
          Map<String, dynamic>.from(existingEntry));
    } else {
    ...
    try {
      ...
      if (cat == 'olhos' || cat == 'eyes') {
        ...
        visualFindings = result.woundDetails ?? {};
          ...
          cat == 'stool' ||
          result.analysisType == 'stool_analysis') {
      ...
      final healthAnalysis = AnaliseFeridaModel(
        ...
        achadosVisuais: visualFindings.isNotEmpty
            ? visualFindings
        ...
        rawClinicalSigns: result.clinicalSignsDiag != null
            ? {'clinical_signs': result.clinicalSignsDiag}
            : null,
      ...
      final alreadyExists = profile.historicoAnaliseFeridas
          .any((e) => e.imagemRef == imagePathToUse);
      if (!alreadyExists) {
          ...
          MaterialPageRoute(
              builder: (_) => EditPetForm(
                  ...
                  onSave: (updated) async {
                    final ps = PetProfileService();
                    await ps.init();
                    await ps.saveOrUpdateProfile(
                        updated.petName, updated.toJson());
  ...
  Future<void> _performAutoSave(PetAnalysisResult result) async {
    ...
    final l10n = AppLocalizations.of(context)!;
    debugPrint(
        '🚀 [AUDIT-RESULT] Iniciando Auto-Save para pet: ${result.petName}');
    try {
      ...
      if (_permanentImage == null) {
        try {
          ...
          final savedPath = await fs.saveMedicalDocument(
              ...
              attachmentType:
                  'health_${result.analysisType}_${DateTime.now().millisecondsSinceEpoch}',
              skipIndexing:
                  true // 🛡️ V_FIX: Avoid duplicate VAULT_UPLOAD in timeline
              );
          if (savedPath != null) {
      ...
      final String safePetName =
          (result.petName != null && result.petName!.trim().isNotEmpty)
              ? result.petName!
              : 'Pet';

      debugPrint(
          '   🔍 [AUDIT-RESULT] Pet Identificado como: "$safePetName" (Type: ${result.analysisType})');

      // 🛡️ [V_UUID] RESOLVE PROFILE EARLY
      final existingEntry =
          await profileService.getProfile(result.petId ?? safePetName);
      PetProfileExtended profile;

      if (existingEntry != null && existingEntry['data'] != null) {
        debugPrint(
            '   ✅ [AUDIT-RESULT] Perfil existente encontrado. ID=${existingEntry['id']}');
        profile = PetProfileExtended.fromHiveEntry(
            Map<String, dynamic>.from(existingEntry));
      } else {
        debugPrint(
            '   🆕 [AUDIT-RESULT] Perfil não encontrado. Criando novo esqueleto...');
        profile = PetProfileExtended.fromAnalysisResult(result, imagePathToUse);
        // If it's a health analysis, DO NOT use the clinical image as the main profile photo
        final bool isClinical = result.analysisType == 'diagnosis' ||
            result.analysisType == 'stool_analysis';
        if (isClinical) {
        ...
        await profileService.saveOrUpdateProfile(
            result.petId ?? safePetName, jsonProfile);
        debugPrint(
            '   ✨ [AUDIT-RESULT] Novo perfil salvo com ID: ${profile.id}');
      }
      ...
      if (result.analysisType == 'diagnosis' ||
          result.analysisType == 'stool_analysis') {
        debugPrint(
            '   🏥 [AUDIT-RESULT] Roteamento: Health Domain (Wound/Stool)');
        ...
        final String cat =
            (result.category ?? result.analysisType).toLowerCase();
        Map<String, dynamic> visualFindings = {};
        if (cat == 'olhos' || cat == 'eyes') {
          visualFindings = result.eyeDetails ?? {};
        } else if (cat == 'dental')
        ...
        profile = profile.copyWith(
          ...
          raca: (profile.raca == null ||
                  profile.raca == 'N/A' ||
                  profile.raca!.contains('SRD'))
              ? result.raca
              : profile.raca,
          imagePath: imagePathToUse,
      ...
      try {
        ...
        if (displayType == 'stool_analysis') {
          displayType = l10n.labCategoryFeces;
        }
        ...
        await indexer.indexAiAnalysis(
          ...
          imagePath: imagePathToUse,
          localizedTitle:
              AppLocalizations.of(context)!.petIndexing_aiTitle(displayType),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(l10n.errorAutoSave(e.toString())),
