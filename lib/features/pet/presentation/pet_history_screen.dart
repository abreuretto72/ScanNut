...
class _PetHistoryScreenState extends ConsumerState<PetHistoryScreen> {
  ...
  Future<void> _loadHistory() async {
    ...
    for (var item in petHistory) {
      ...
      final rawId =
          item['pet_id'] ?? item['id'] ?? data['pet_id'] ?? data['id'];
        ...
        if (!profileMap.containsKey(identifier)) {
          ...
          if (profileData != null && profileData['data'] != null) {
            try {
              ...
              debugPrint(
                  '❌ [PetHistory] Error parsing profile for $identifier: $e');
            }
          }
        }

        // Load Vet for specific entries if needed
        if (item['type'] == 'pet_profile_created' ||
            item['type'] == 'pet_profile_updated') {
  ...
  Widget build(BuildContext context) {
    ...
    if (_isLoading) {
      return Scaffold(
        ...
        body: const Center(
            child: CircularProgressIndicator(color: AppDesign.accent)),
      );
    }
      ...
      body: !Hive.isBoxOpen(HistoryService.boxName)
          ...
          : ValueListenableBuilder<Box>(
              ...
              builder: (context, box, _) {
                ...
                debugPrint(
                    '📜 [AUDIT] Total de itens brutos na history box: ${allItems.length}');

                final rawHistory = allItems
                    .where((item) {
                      if (item is! Map) {
                        debugPrint(
                            '   ⚠️ [AUDIT] Item ignorado: não é um Map (${item.runtimeType})');
                        return false;
                      }

                      final mode = item['mode']?.toString().toLowerCase();
                      final hasPetId = item['pet_id'] != null ||
                          (item['data'] is Map &&
                              item['data']['pet_id'] != null);
                      final hasPetName = item['pet_name'] != null ||
                          (item['data'] is Map &&
                              item['data']['pet_name'] != null);

                      // 🛡️ Permissive Filter: Accept explicit 'pet' mode OR any item with pet identifiers
                      final isPet = mode == 'pet' ||
                          (mode == null && (hasPetId || hasPetName));

                      if (!isPet) {
                        debugPrint(
                            '   ℹ️ [AUDIT] Item filtrado (não identificado como Pet): mode=$mode');
                      }
                      return isPet;
                    })
                    .map((e) => deepCastMap(e))
                ...
                debugPrint(
                    '📜 [AUDIT] Itens após filtro de mode=Pet: ${rawHistory.length}');
                ...
                for (var item in rawHistory) {
                  ...
                  final rawId = item['pet_id'] ??
                      item['id'] ??
                      data['pet_id'] ??
                      data['id'];
                  final rawName =
                      item['pet_name'] ?? data['pet_name'] ?? data['name'];

                  final key = rawId?.toString() ??
                      (rawName?.toString().trim().isNotEmpty == true
                          ? rawName.toString().trim()
                          : 'Unknown_${item['timestamp']}');

                  debugPrint(
                      '   🔍 [AUDIT] Processando Pet: Name=$rawName, ID=$rawId -> Final Key=$key');

                  if (key.isNotEmpty) {
                    if (!uniqueMap.containsKey(key)) {
                      uniqueMap[key] = item;
                    } else {
                      final currentTs = DateTime.tryParse(
                              uniqueMap[key]?['timestamp'] ?? '') ??
                          DateTime(2000);
                      final newTs =
                    ...
                    child: Column(
                      ...
                      children: [
                        const Icon(Icons.pets,
                            size: 64, color: AppDesign.petPink),
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return GestureDetector(
                        onTap: () async {
                          ...
                          final authResult =
                              await SimpleAuthService.authenticate();
                          if (authResult != AuthResult.success) return;

                          try {
                            final analysisData = deepCastMap(data);
                            final result =
                                PetAnalysisResult.fromJson(analysisData);
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            boxShadow: [
                              BoxShadow(
                                color: Colors.black
                                    .withValues(alpha: 0.2), // Linter Fix
                                blurRadius: 10,
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Row(
                                  ...
                                  children: [
                                    ...
                                    IconButton(
                                      ...
                                      onPressed: () {
                                        final profile =
                                            _petProfiles[identifier];
                                        Navigator.push(
                                          context,
                                          MaterialPageRoute(
                                            builder: (context) => PetChatScreen(
                                              petId: identifier,
                                              petName: petName,
                                              profile: profile,
                                            ),
                                ...
                                PetActionBar(
                                  petId: identifier,
                                  petName: petName,
                                  onAgendaTap: () => _showEventSelector(
                                      context, identifier, petName),
                                  onMenuTap: () => _handleMenuTap(
                                      context, identifier, petName),
                                  onEditTap: () => _handleEditTap(
                                      item, identifier, petName, data),
                                ),
                              ],
                            ),
                          ),
  ...
  void _showEventSelector(
      BuildContext context, String identifier, String petName) {
    final l10n = AppLocalizations.of(context)!;
                      ...
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (context) => PetEventHistoryScreen(
                              petId: identifier, petName: petName),
                        ),
                      );
                    },
                    icon: const Icon(Icons.history,
                        size: 16, color: Colors.black),
                    label: Text(l10n.petShowAll,
                        style: GoogleFonts.poppins(
  ...
  Widget _buildPetAvatar(Map<String, dynamic> item, {double radius = 28}) {
    ...
    final rawName = item['pet_name'] ??
        (item['data'] is Map ? item['data']['pet_name'] : null) ??
        (item['data'] is Map ? item['data']['name'] : null);
    final rawId = item['pet_id'] ??
        item['id'] ??
        (item['data'] is Map
            ? (item['data']['pet_id'] ?? item['data']['id'])
            : null);
    ...
    if (!hasValidImage && imagePath != null && _appDocsDir != null) {
      ...
      try {
        ...
        if (!rFile.existsSync()) {
          rPath =
              path.join(_appDocsDir!.path, 'medical_docs', petName, filename);
          rFile = File(rPath);
        }
    ...
    if (hasValidImage) {
      ...
      debugPrint(
          '   ℹ️ [UI_TRACE] No valid image for $petName - using fallback');
    }
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        shape: BoxShape.circle,
        border: Border.all(
            color: AppDesign.petPink.withValues(alpha: 0.3), width: 1),
      ),
      child: ClipOval(
        child: hasValidImage
            ? Image.file(
  ...
  Future<void> _handleEditTap(Map<String, dynamic> item, String identifier,
      String petName, dynamic data) async {
    ...
    try {
      ...
      if (existingMap != null && existingMap['data'] != null) {
        ...
        debugPrint(
            '⏱️ [PERF] deepCastMap (Cost): ${sw.elapsedMilliseconds - startParse}ms');

        final startJson = sw.elapsedMilliseconds;
        loaded = PetProfileExtended.fromJson(dataMap);
        debugPrint(
            '⏱️ [PERF] fromJson (Cost): ${sw.elapsedMilliseconds - startJson}ms');
      }
    } catch (e) {
        ...
        builder: (context) => EditPetForm(
          existingProfile: loaded ??
              PetProfileExtended(
                ...
                petName: petName,
                raca: data['breed'] ??
                    data['identificacao']?['raca_predominante'],
                nivelAtividade:
                    (data['perfil_comportamental']?['nivel_energia'] is int)
                        ? ((data['perfil_comportamental']!['nivel_energia']
                                    as int) >
                                3
                            ? 'Ativo'
                            : ((data['perfil_comportamental']!['nivel_energia']
                                ...
                                : 'Moderado'))
                        : 'Moderado',
                frequenciaBanho: data['higiene']?['banho_e_higiene']
                            ?['frequencia']
                        ?.toString() ??
                    'Quinzenal',
          ...
          onSave: (profile) async {
            ...
            if (profile.raca != null) {
              if (analysisData['identificacao'] == null) {
                analysisData['identificacao'] = {};
              }
  ...
  Future<void> _handleMenuTap(
      BuildContext context, String identifier, String petName) async {
