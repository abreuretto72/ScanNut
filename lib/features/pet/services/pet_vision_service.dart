...
class PetVisionService {
  ...
  Future<Map<String, dynamic>> analisarFotoPet(File imageFile, String locale,
      {String? knownSpecies,
      String? knownBreed,
      ScannutMode mode = ScannutMode.petIdentification}) async {
    try {
      ...
      if (knownSpecies != null || knownBreed != null) {
        final aiSpecies =
            result['species'] ?? result['identification']?['species'];
        final aiBreed = result['breed'] ?? result['identification']?['breed'];

        if (aiSpecies != null && aiSpecies.toString().toLowerCase() != 'n/a') {
          // ignore: avoid_print
          print(
              '🛡️ DEBUG: [SOURCE OF TRUTH BREACH] AI attempted to return species: $aiSpecies');
        }
        if (aiBreed != null && aiBreed.toString().toLowerCase() != 'n/a') {
          // ignore: avoid_print
          print(
              '🛡️ DEBUG: [SOURCE OF TRUTH BREACH] AI attempted to return breed: $aiBreed');
        }
      }
      ...
      if (mutableResult.containsKey('identification') &&
          mutableResult['identification'] is Map) {
        if (knownSpecies != null) {
          mutableResult['identification']['species'] = knownSpecies;
        }
        if (knownBreed != null) {
          mutableResult['identification']['breed'] = knownBreed;
        }
      }
