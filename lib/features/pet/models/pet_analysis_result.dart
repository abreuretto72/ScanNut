...
class PetAnalysisResult {
  ...
  final Map<String, dynamic>?
      clinicalSignsDiag; // 🛡️ V139: Added for Deep Scan
  final Map<String, dynamic>?
      stoolAnalysis; // 🛡️ V231: Stool Analysis specialized data
  ...
  String get raca =>
      analysisType == 'diagnosis' || analysisType == 'stool_analysis'
          ? (racaDiag ?? 'N/A')
          : identificacao.racaPredominante;
  String get especie =>
      analysisType == 'diagnosis' || analysisType == 'stool_analysis'
          ? (especieDiag ?? 'Animal')
          : (especieId ?? "Animal");
  String get caracteristicas =>
      analysisType == 'diagnosis' || analysisType == 'stool_analysis'
          ? (caracteristicasDiag ?? 'N/A')
          : identificacao.porteEstimado;
  String get descricaoVisual =>
      analysisType == 'diagnosis' || analysisType == 'stool_analysis'
          ? (descricaoVisualDiag ?? 'N/A')
          : perfilComportamental.driveAncestral;
  String get urgenciaNivel =>
      analysisType == 'diagnosis' || analysisType == 'stool_analysis'
          ? (urgenciaNivelDiag ?? 'Verde')
          : "Verde";
  String get orientacaoImediata =>
      analysisType == 'diagnosis' || analysisType == 'stool_analysis'
          ? (orientacaoImediataDiag ?? 'Consulte um Vet.')
          : dica.insightExclusivo;
  ...
  factory PetAnalysisResult.fromJson(Map<String, dynamic> json) {
    ...
    if (json['analysis_type'] == 'diagnosis' ||
      ...
      return PetAnalysisResult(
        ...
        possiveisCausasDiag: (json['possible_causes'] as List? ?? [])
            .map((e) => e.toString())
            .toList(),
        clinicalSignsDiag: json['clinical_signs'] != null
            ? Map<String, dynamic>.from(json['clinical_signs'])
            : null,
        stoolAnalysis: json['stool_details'] != null
            ? Map<String, dynamic>.from(json['stool_details'])
            : null, // 🛡️ V231
        category: json['category'], // 🛡️ V460
        eyeDetails: json['eye_details'] != null
            ? Map<String, dynamic>.from(json['eye_details'])
            : null, // 🛡️ V460
        dentalDetails: json['dental_details'] != null
            ? Map<String, dynamic>.from(json['dental_details'])
            : null, // 🛡️ V460
        skinDetails: json['skin_details'] != null
            ? Map<String, dynamic>.from(json['skin_details'])
            : null, // 🛡️ V460
    ...
    return PetAnalysisResult(
      ...
      especieId:
          idMap['species']?.toString(), // Capture species from identification
...
class IdentificacaoPet {
  ...
  factory IdentificacaoPet.fromTotalInference(
      Map<String, dynamic> id, Map<String, dynamic> growth) {
    String size = id['size']?.toString() ?? 'Medium';
    Map<String, dynamic> fallbackGrowth =
        PetFallbackService.getGrowthCurve(size);
    ...
    String? check(dynamic val, String key) {
      ...
      if (val == null ||
        ...
        return fallbackValue != null
            ? '$fallbackValue [ESTIMATED]'
            : 'Estimated [ESTIMATED]';
      }
      return val.toString();
    }

    mappedGrowth['peso_3_meses'] =
        check(growth['weight_3_months'], 'weight_3_months');
    mappedGrowth['peso_6_meses'] =
        check(growth['weight_6_months'], 'weight_6_months');
    mappedGrowth['peso_12_meses'] =
        check(growth['weight_12_months'], 'weight_12_months');
    ...
    String? getString(List<String> keys) {
      for (final k in keys) {
        if (id[k] != null &&
            id[k].toString().isNotEmpty &&
            id[k].toString().toLowerCase() != 'null') {
    ...
    final morfologia = getString([
        ...
        '';
    final linhagem =
        getString(['lineage', 'linhagem', 'linhagem_provavel', 'ancestry']) ??
            'Pure/Standard';
    final life = getString(['lifespan', 'expectativa_vida', 'longevidade']) ??
        '10-12 years';
  ...
  factory IdentificacaoPet.fromJson(Map<String, dynamic> json) {
    ...
    return IdentificacaoPet(
      ...
      linhagemSrdProvavel:
          json['linhagemSrdProvavel'] ?? json['linhagem'] ?? '',
...
class NutricaoEStrutura {
  ...
  factory NutricaoEStrutura.fromTotalInference(
      Map<String, dynamic> json, String size) {
    Map<String, String> meta = {};
    Map<String, String> fallback =
        PetFallbackService.getNutritionalTargets(size);
    ...
    return NutricaoEStrutura(
      metaCalorica: meta,
      nutrientesAlvo: (json['target_nutrients'] as List? ?? [])
          .map((e) => e.toString())
          .toList(),
  ...
  factory NutricaoEStrutura.fromJson(Map<String, dynamic> json) =>
      NutricaoEStrutura.empty();
  factory NutricaoEStrutura.fromSimplifiedJson(Map<String, dynamic> json) =>
      NutricaoEStrutura.empty();
  factory NutricaoEStrutura.fromUnifiedJson(Map<String, dynamic> json) =>
      NutricaoEStrutura.empty();
  factory NutricaoEStrutura.empty() => NutricaoEStrutura(
      metaCalorica: {},
      nutrientesAlvo: [],
      suplementacaoSugerida: [],
      segurancaAlimentar: {});
...
class Grooming {
  ...
  factory Grooming.fromTotalInference(Map<String, dynamic> json) {
    ...
    String freq = check(json['grooming_frequency'],
        '${PetFallbackService.getGroomingFrequency(type)} [ESTIMATED]');
  ...
  factory Grooming.fromUnifiedJson(
          Map<String, dynamic> a, Map<String, dynamic> b) =>
      Grooming.empty();
  factory Grooming.empty() =>
      Grooming(manutencaoPelagem: {}, banhoEHigiene: {});
  Map<String, dynamic> toJson() => {
...
class SaudePreventiva {
  ...
  factory SaudePreventiva.fromTotalInference(Map<String, dynamic> json) {
    List<String> predis = (json['predispositions'] as List? ?? [])
        .map((e) => e.toString())
        .toList();
    ...
    return SaudePreventiva(
      ...
      checkupVeterinario: {
        'exames_obrigatorios_anuais': [
          json['preventive_checkup'] ?? 'Hemograma e Checkup Geral'
        ]
      },
      sensibilidadeClimatica: {},
      predisposicoesGeneticas: predis.join(", "), // 🚀 V125
      sinaisAlertaPrecoce:
          json['early_warning_signs']?.toString() ?? '', // 🚀 V125
    );
  }
  ...
  factory SaudePreventiva.fromJson(Map<String, dynamic> json) =>
      SaudePreventiva.empty();
  factory SaudePreventiva.fromSimplifiedJson(Map<String, dynamic> json) =>
      SaudePreventiva.empty();
  factory SaudePreventiva.fromUnifiedJson(Map<String, dynamic> json) =>
      SaudePreventiva.empty();
  Map<String, dynamic> toJson() => {
        'predispositions': predisposicaoDoencas,
        'preventive_checkup': (checkupVeterinario['exames_obrigatorios_anuais']
                    is List &&
                (checkupVeterinario['exames_obrigatorios_anuais'] as List)
                    .isNotEmpty)
            ? (checkupVeterinario['exames_obrigatorios_anuais'] as List).first
            : null,
      };
}
...
class LifestyleEEducacao {
  ...
  factory LifestyleEEducacao.fromTotalInference(Map<String, dynamic> json) {
    return LifestyleEEducacao(treinamento: {
      ...
      'adaptacao_apartamento_score': (json['environment_type']
                  ?.toString()
                  .toLowerCase()
                  .contains('apart') ??
              false)
  ...
  factory LifestyleEEducacao.empty() => LifestyleEEducacao(
      treinamento: {}, ambienteIdeal: {}, estimuloMental: {});
  factory LifestyleEEducacao.fromJson(Map<String, dynamic> json) =>
      LifestyleEEducacao.empty();
  factory LifestyleEEducacao.fromSimplifiedJson(Map<String, dynamic> json) =>
      LifestyleEEducacao.empty();
  factory LifestyleEEducacao.fromUnifiedJson(Map<String, dynamic> json) =>
      LifestyleEEducacao.empty();
  Map<String, dynamic> toJson() => {
...
class DicaEspecialista {
  ...
  factory DicaEspecialista.fromJson(Map<String, dynamic> json) =>
      DicaEspecialista.empty();
  factory DicaEspecialista.fromSimplifiedJson(dynamic json) =>
      DicaEspecialista.empty();
  factory DicaEspecialista.fromUnifiedJson(dynamic json) =>
      DicaEspecialista.empty();
  Map<String, dynamic> toJson() => {};
}
