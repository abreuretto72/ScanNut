...
class _PlantExportConfigurationModalState extends State<PlantExportConfigurationModal> {
    ...
    if (_selectedDateRange != null) {
      items = items.where((item) {
        return item.timestamp.isAfter(_selectedDateRange!.start.subtract(const Duration(seconds: 1))) &&
               item.timestamp.isBefore(_selectedDateRange!.end.add(const Duration(days: 1)));
      }).toList();
    }
    ...
    switch (_filterMode) {
      ...
      case 'diseased':
        return items.where((item) => 
          item.diseaseDiagnosis != null && item.diseaseDiagnosis!.isNotEmpty
        ).toList();
      case 'manual':
        return items;
      case 'all':
          ...
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                   Expanded(child: Text('Exportar Inteligência Botânica', style: GoogleFonts.poppins(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold))),
              ...
              InkWell(
                ...
                child: Container(
                  ...
                  child: Row(
                    children: [
                      const Icon(Icons.calendar_today, color: AppDesign.plantGreen, size: 20),
              ...
              // FILTER OPTIONS
              Text('Filtrar por:', style: GoogleFonts.poppins(color: Colors.grey, fontSize: 12)),
              const SizedBox(height: 10),
              
              _buildFilterOption('Todas as Plantas', 'all', Icons.local_florist),
              const SizedBox(height: 8),
              _buildFilterOption('Apenas Tóxicas', 'toxic', Icons.warning_amber_rounded),
              const SizedBox(height: 8),
              _buildFilterOption('Apenas Seguras', 'safe', Icons.check_circle_outline),
              const SizedBox(height: 8),
              _buildFilterOption('Apenas com Doenças', 'diseased', Icons.healing),
              const SizedBox(height: 8),
              ...
              if (_filterMode == 'manual') ...[
                 ...
                 Container(
                   ...
                   child: filteredList.isEmpty 
                       ? const Center(child: Text('Nenhuma planta encontrada.', style: TextStyle(color: Colors.white30)))
                       : ListView.builder(
                           shrinkWrap: true,
                           ...
                           itemBuilder: (context, index) {
                             ...
                             return CheckboxListTile(
                               title: Text(item.plantName, style: const TextStyle(color: Colors.white, fontSize: 13)),
                               subtitle: Row(
                                 children: [
                                   Text(DateFormat('dd/MM HH:mm').format(item.timestamp), style: const TextStyle(color: Colors.white54, fontSize: 11)),
                                   const SizedBox(width: 8),
                                   Icon(
                                     isToxic ? Icons.warning_amber_rounded : Icons.check_circle_outline,
                                     size: 14,
                                     color: isToxic ? Colors.redAccent : Colors.greenAccent,
                                   ),
                                 ],
                               ),
                               value: isSelected,
  ...
  Widget _buildFilterOption(String label, String value, IconData icon) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Icon(icon, color: isSelected ? AppDesign.plantGreen : Colors.white54, size: 20),
            const SizedBox(width: 10),
            Text(
              label,
