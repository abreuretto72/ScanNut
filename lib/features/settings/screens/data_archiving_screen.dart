...
class _DataManagerScreenState extends ConsumerState<DataManagerScreen> {
        ...
        try {
          if (Hive.isBoxOpen(name)) {
            try {
              ...
              debugPrint(
                  'ℹ️ [Stats] Skipping open box $name due to type/access error.');
            }
          } else {
            if (await Hive.boxExists(name)) {
              if (cipher != null) {
                try {
                  ...
                  debugPrint(
                      '⚠️ [Stats] Failed to open $name for counting: $e');
                }
              }
            }
          }
    ...
    try {
      ...
      if (await dir.exists()) {
        final files = dir
            .listSync()
            .whereType<File>()
            .where((f) => f.path.endsWith('.scannut'))
            .toList();
        if (files.isNotEmpty) {
          ...
          if (mounted) {
            setState(() {
              ...
              _lastBackupFileName =
                  lastFile.path.split('/').last.split('\\').last;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: Text('Gerenciador de Dados',
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Icon(Icons.history,
                            color: _lastBackupDate == "Sem Backup"
                                ? Colors.orange
                                : Colors.green,
                            size: 20),
                        const SizedBox(width: 8),
                        Text('Último Backup Permanente',
                            style: GoogleFonts.poppins(
                                color: Colors.white,
    ...
    return Column(
      children: [
        Row(
          children: [
            Expanded(
                child: _buildStatCard('Espaço Total', '$_totalSizeMB MB',
                    Icons.sd_storage, Colors.blue)),
            const SizedBox(width: 8),
            Expanded(
                child: _buildStatCard('Mídia (Anexos)', '$_mediaSizeMB MB',
        ...
        Row(
          children: [
            Expanded(
                child: _buildStatCard('Banco de Dados', '$_dbSizeMB MB',
                    Icons.storage, Colors.orange)),
            const SizedBox(width: 8),
            Expanded(
                child: _buildStatCard('Itens Registrados',
                    '$_countOccurrences+', Icons.list_alt, Colors.green)),
          ],
        ),
      ],
    );
        ...
        children: [
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              Text('Arquivamento com Filtro 3D',
                  style: GoogleFonts.poppins(
          ...
          // FILTERS PREVIEW
          _buildFilterRow(
              'QUEM', _selectedPetName ?? 'Todos os Pets', Icons.pets),
          if (_selectedPetId != null && _selectedPetId != _selectedPetName) ...[
            const SizedBox(height: 4),
            _buildFilterRow('ID', '${_selectedPetId!.substring(0, 8)}...',
                Icons.fingerprint),
          ],
          const SizedBox(height: 8),
          _buildFilterRow(
              'O QUÊ',
              '${_categories.entries.where((e) => e.value).length} Categorias',
              Icons.category),
          const SizedBox(height: 8),
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: const Text('Arquivar Dados (Filtro Personalizado)',
  ...
  Widget _buildAttachmentManagerCard() {
    return InkWell(
      onTap: () => Navigator.push(context,
          MaterialPageRoute(builder: (_) => const MediaManagerScreen())),
      borderRadius: BorderRadius.circular(16),
      child: Container(
        padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text('Gerenciar arquivos pesados',
                      style: GoogleFonts.poppins(
    ...
    return Column(
      children: [
        _buildDomainDeleteCard(
            ...
            actions: [
              _buildDeleteAction(
                  label: 'Excluir Total (Dados + Fotos)',
                  onTap: () =>
                      _confirmAction('Tudo de Alimentos', _wipeFoodData)),
            ]),
        const SizedBox(height: 16),
        _buildDomainDeleteCard(
            title: 'PLANTAS',
            ...
            actions: [
              _buildDeleteAction(
                  label: 'Excluir Total (Dados + Fotos)',
                  onTap: () =>
                      _confirmAction('Tudo de Plantas', _wipePlantData)),
            ]),
        const SizedBox(height: 16),
        _buildDomainDeleteCard(
            title: 'PETS',
            ...
            actions: [
              _buildDeleteAction(
                  label: 'Excluir Total (Dados + Fotos)',
                  onTap: () => _confirmAction('Tudo de Pets', _wipePetData)),
            ]),
        const SizedBox(height: 16),
        _buildDomainDeleteCard(
            title: 'GERAL',
            ...
            actions: [
              _buildDeleteAction(
                  label: 'Excluir TUDO (Reset de Fábrica)',
                  isDestructive: true,
                  onTap: () => _confirmAction(
                      'CONTA COMPLETA', () => _performFactoryReset())),
            ]),
      ],
    );
  }
  ...
  Widget _buildDeleteAction(
    ...
    return ListTile(
      ...
      trailing: Icon(Icons.delete_outline,
          color: isDestructive ? Colors.red : AppDesign.textSecondaryDark,
          size: 20),
      onTap: onTap,
    );
  }
    ...
    if (!isVerified) {
      if (mounted) {
        SnackBarHelper.showError(
            context, 'Autenticação falhou. Ação cancelada.');
      }
      return;
    }
  ...
  Future<void> _executeSmartArchiving(String? petName, String? petId,
      Map<String, bool> categories, DateTimeRange? dateRange) async {
    ...
    try {
      ...
      final toDelete = allEvents.where((e) {
        ...
        if (targetName != null && e.petName.trim().toLowerCase() == targetName) {
          return true;
        }
        return false;
      }).where((e) {
        if (dateRange != null) {
          if (e.dateTime.isBefore(dateRange.start) ||
              e.dateTime.isAfter(dateRange.end.add(const Duration(days: 1)))) {
            return false;
          }
        }
        return true;
      ...
      if (mounted) {
        ...
        SnackBarHelper.showSuccess(context,
            'Filtro 3D: $count registros de $petName excluídos/arquivados.');
      }
    } catch (e) {
      if (mounted) {
        Navigator.pop(context);
  ...
  Future<void> _performFactoryReset() async {
    ...
    try {
      ...
      debugPrint(
          '🛡️ [Factory Reset] Trace: Encryption Cipher is ${cipher != null ? "ACTIVE" : "NULL/INACTIVE"}');

      // 🚀 STEP 0: CLOSE ALL BOXES
      // This is critical to avoid "Box already open with different type" errors during reset
      debugPrint(
          '🧹 [Factory Reset] Step 0: Closing all boxes to ensure clean deletion...');
      await Hive.close();
      ...
      for (final name in boxesToClear) {
        try {
          ...
          if (await Hive.boxExists(name)) {
            ...
            debugPrint(
                '✅ [Factory Reset] NUCLEAR DELETE for $name (Disk files removed)');
          } else {
            debugPrint(
                'ℹ️ [Factory Reset] Box $name does not exist on disk, skipping.');
          }
        } catch (e) {
    ...
    try {
      ...
      debugPrint(
          '🔄 [Factory Reset] Providers invalidated. UI should refresh.');
    } catch (e) {
  ...
  Future<void> _wipePetData() async {
    ...
    await _clearHistoryDeep((v) =>
        v['mode'] == 'Pet' || (v['pet_name'] != null) || (v['petId'] != null));
...
class _FilterModalContent extends StatefulWidget {
  ...
  final Function(String?, String?, Map<String, bool>, DateTimeRange?, String)
      onApply;
...
class _FilterModalContentState extends State<_FilterModalContent> {
  ...
  bool get _isValid =>
      _selectedPetId != null && _localCategories.values.contains(true);
                        ...
                        child: DropdownButtonHideUnderline(
                          child: DropdownButton<String>(
                            ...
                            onChanged: (val) {
                              setState(() {
                                _selectedPetId = val;
                                _selectedPetName = summaries.entries
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: _isValid
                    ? () {
                        Navigator.pop(context);
                        widget.onApply(_selectedPetName, _selectedPetId,
                            _localCategories, _dateRange, _dateLabel);
                      }
                    : null,
                style: ElevatedButton.styleFrom(
                  backgroundColor: Colors.white,
                ...
                child: Text('Exportar e Liberar Espaço',
                    style: GoogleFonts.poppins(
