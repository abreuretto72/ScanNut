...
class ShoppingListService {
  ...
  List<WeeklyShoppingList> generateWeeklyListsFromPlan(WeeklyPlan plan) {
    ...
    for (int w = 0; w < numWeeks; w++) {
      ...
      final weekDays = days.sublist(
        startIdx, 
        endIdx > days.length ? days.length : endIdx
      );
  ...
  WeeklyShoppingList _generateSingleWeeklyList({
    ...
    final categories = [
      ShoppingListCategory(id: 'hortifruti', title: 'Hortifruti', order: 1, items: categoriesMap['hortifruti']!..sort((a,b) => a.name.compareTo(b.name))),
      ShoppingListCategory(id: 'proteinas', title: 'Proteínas', order: 2, items: categoriesMap['proteinas']!..sort((a,b) => a.name.compareTo(b.name))),
      ShoppingListCategory(id: 'laticinios', title: 'Laticínios', order: 3, items: categoriesMap['laticinios']!..sort((a,b) => a.name.compareTo(b.name))),
      ShoppingListCategory(id: 'graos', title: 'Grãos e Cereais', order: 4, items: categoriesMap['graos']!..sort((a,b) => a.name.compareTo(b.name))),
      ShoppingListCategory(id: 'mercearia', title: 'Mercearia / Secos', order: 5, items: categoriesMap['mercearia']!..sort((a,b) => a.name.compareTo(b.name))),
      ShoppingListCategory(id: 'outros', title: 'Outros', order: 6, items: categoriesMap['outros']!..sort((a,b) => a.name.compareTo(b.name))),
    ];
  ...
  void _processItem(MealItem item, Map<String, _ConsolidatedItem> aggregator) {
    ...
    _aggregate(primaryName, item.quantidadeTexto, aggregator, isOptional: false);
  ...
  void _aggregate(String rawName, String rawQty, Map<String, _ConsolidatedItem> aggregator, {required bool isOptional}) {
    String name = rawName.trim().toLowerCase();
    name = name.replaceAll('porção de ', '').replaceAll('porcao de ', '').replaceAll('porção', '').trim();
  ...
  ShoppingQuantity _finalizeQuantity(Map<String, double> quantities) {
    ...
    if (quantities.containsKey('kg')) return ShoppingQuantity(value: quantities['kg']!, unit: 'kg');
    if (quantities.containsKey('g')) {
      double totalG = quantities['g']!;
      if (totalG >= 1000) return ShoppingQuantity(value: totalG / 1000, unit: 'kg');
      return ShoppingQuantity(value: totalG, unit: 'g');
    }
    if (quantities.containsKey('L')) return ShoppingQuantity(value: quantities['L']!, unit: 'L');
    if (quantities.containsKey('ml')) {
      double totalMl = quantities['ml']!;
      if (totalMl >= 1000) return ShoppingQuantity(value: totalMl / 1000, unit: 'L');
  ...
  String _formatQuantityDisplay(ShoppingQuantity qty) {
    final val = qty.value % 1 == 0 ? qty.value.toInt().toString() : qty.value.toStringAsFixed(1);
    return '$val ${qty.unit}';
  }

  String _classifyCategory(String name) {
    final lower = name.toLowerCase();
    if (_containsAny(lower, ['alface', 'tomate', 'cebola', 'alho', 'fruta', 'maçã', 'banana', 'limão', 'cenoura', 'batata', 'brocolis', 'brócolis', 'couve'])) return 'hortifruti';
    if (_containsAny(lower, ['frango', 'carne', 'boi', 'peixe', 'tilápia', 'ovo', 'clara', 'bife', 'file', 'presunto'])) return 'proteinas';
    if (_containsAny(lower, ['leite', 'queijo', 'iogurte', 'manteiga', 'requeijão', 'ricota'])) return 'laticinios';
    if (_containsAny(lower, ['arroz', 'feijão', 'lentilha', 'grao', 'farinha', 'aveia'])) return 'graos';
    if (_containsAny(lower, ['azeite', 'óleo', 'vinagre', 'sal', 'pimenta', 'açúcar', 'café', 'pão', 'chocolate', 'castanha'])) return 'mercearia';
    return 'outros';
  }
